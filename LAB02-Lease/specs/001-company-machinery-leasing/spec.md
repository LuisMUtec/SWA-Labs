...
- **AC-012**: **Given** a Lease, **When** Company requests the count and total amount of its `paid`, `due` and `pending` Installments, **Then** the three counts always sum to the Lease's total number of Installments and the three amounts always sum to the Lease's total obligation, with no Installment falling outside all three. *(FR-014)*
