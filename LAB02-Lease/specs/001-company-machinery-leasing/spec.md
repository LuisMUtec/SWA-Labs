...
- **Financing Decision**: the resolution of a Leasing Request — whether Lease Company will finance it or not. Its possible outcomes are in scope, as is the reason recorded with a refusal reaching Company (FR-007); *how* Lease Company arrives at an outcome is not.
...
- **AC-005c**: **Given** a Leasing Request refused with a recorded reason (`002` FR-013), **When** Company retrieves that request, **Then** the reason is retrievable by Company alongside the `rejected` status. *(FR-007)*
...
- **AC-010b**: **Given** an Installment that is still `pending` because its Certification Milestone has not been certified, **When** Company attempts to pay it, **Then** the system rejects the attempt — an installment is not payable before the project progress it is anchored to has been certified (BR-04). *(FR-012)*
...
- **AC-014b**: **Given** an `available` Acquisition Option, **When** Company exercises it, **Then** the system requires no payment beyond the Installments already paid — the option to acquire costs nothing further once every installment is settled (BR-07). *(FR-017)*
...
- **AC-016d**: **Given** a Leasing Operation whose Acquisition Option Company has exercised, **When** Company retrieves it, **Then** the Machinery is recorded as Company's and Lea$e's ownership of it as ended (BR-01, BR-07). *(FR-017)*
...
- **AC-028d**: **Given** a requirement to restore cover raised against its own Leasing Operation, **When** Company retrieves that operation, **Then** whether the requirement is still outstanding or has been met is retrievable to it. *(FR-029)*
...
- **FR-007**: When a Leasing Request's status becomes `rejected`, the system MUST make that status observable to Company, MUST make the reason recorded with the refusal (`002` FR-013) retrievable by Company alongside it, and MUST NOT create a Lease for that request. **Amended 2026-08-21, per EVAL iteration 09:** `002` FR-013 was amended to make a refusal's reason observable to the Applicant and named this requirement as where it lands, and this requirement carried only the status — a demand terminating at the other spec's edge, which is the defect iteration 07 charged half a point for at `002` FR-030. What is in scope here is that the recorded reason reaches Company; *how* Lea$e arrives at an outcome stays out of scope, as Key Product Concepts says.
...
- **FR-017**: The system MUST allow Company to exercise an `available` Acquisition Option at no cost beyond the Installments already paid (BR-07), MUST record that the Machinery has passed to Company and that Lea$e's ownership of it has ended (BR-01), and MUST bring the corresponding Leasing Operation to an unambiguous completed state once exercised. **Amended 2026-08-21, per EVAL iteration 08:** that ownership passes was stated only in Key Product Concepts — *"the mechanism through which BR-01's 'Lea$e retains ownership' ends"* — and in no requirement and no acceptance criterion, so the single outcome `personas/Pedro.MD` names as its success signal, that the operation "ends in a state that leaves no doubt that the machinery is his", rested on a claim nothing in the requirement set made. This is the load-bearing-statement-outside-the-requirements defect the project has charged three times over figures; here it was an outcome.
