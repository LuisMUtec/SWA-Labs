...
- **What constitutes a completed Leasing Operation, and what happens if Company never exercises**: for Stage 1, a Leasing Operation is complete once Company exercises an available Acquisition Option and the system confirms the exercise. **Resolved (2026-08-21):** an available Acquisition Option that Company has not exercised is not left ambiguous — it is the named, non-terminal state `available, not yet exercised`, and this feature does not force a decision within any deadline. Company may exercise it at any later point and nothing about the state itself is undetermined. What happens to the Lease and the Machine if it is *never* exercised — a term boundary, a return trigger — is not this feature's behavior to define: [`003-deployed-fleet-custody`](../003-deployed-fleet-custody/spec.md) already closes a Deployment by `Return` whenever its operation's Acquisition Option has not been exercised (BR-07), independently of any action Company takes here. `001` and `003` therefore agree on the outcome (Returned when not exercised) without `001` needing to add a mechanism of its own for it.
...
- `[ASSUMPTION]` An available Acquisition Option that Company has not exercised is a stable, named, non-terminal state (`available, not yet exercised`), not a defect; this feature sets no deadline for exercising it. Whether and when a Lease closes by `Return` in that case is `003-deployed-fleet-custody`'s behavior, not this feature's.
