...
- **What constitutes a completed Leasing Operation, and what happens if Company never exercises**: for Stage 1, a Leasing Operation is complete once Company exercises an available Acquisition Option and the system confirms the exercise. **Resolved (2026-08-21):** an available Acquisition Option that Company has not exercised is not left ambiguous — it is the named, non-terminal state `available, not yet exercised`. This feature does not itself decide *when* a Lease's term ends or what forces a final decision by then — that boundary belongs to [`003-deployed-fleet-custody`](../003-deployed-fleet-custody/spec.md), which closes a Deployment by exactly one of `Return` or `Acquisition Retirement` once its operation reaches that point, determined by whether the Acquisition Option was exercised at that time (BR-07). What `001` fixes is only the two facts `003` reads to make that determination: the Option's own availability, and whether Company has exercised it. Company may exercise an available Option at any point up to whenever the Lease's term ends; this feature does not narrow that further, and neither does `003` — the moment itself is common, unresolved scope in both, not a contradiction between them.
...
- What happens to the Lease and the Machine when the Acquisition Option is never exercised — that closing behavior belongs to `003-deployed-fleet-custody`'s `Return`, not to this feature.
...
- `[ASSUMPTION]` An available Acquisition Option that Company has not exercised is a stable, named, non-terminal state (`available, not yet exercised`), not a defect. When a Lease's term ends, and what forces the exercise-or-not decision by then, is unresolved scope shared with `003-deployed-fleet-custody` (see its own `[CLARIFY]`-adjacent note in Key Product Decisions) rather than something this feature invents unilaterally.
