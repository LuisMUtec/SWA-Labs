...
- **Installment**: one payment obligation belonging to a Lease. Has a status (`pending` / `paid`) and an amount. A Lease's Installments collectively determine when its Acquisition Option becomes available.
...
9. Company can view the Lease's Installments, each one's status and each one's amount.
