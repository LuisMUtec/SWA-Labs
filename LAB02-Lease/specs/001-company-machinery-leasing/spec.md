...
- **AC-006**: **Given** an approved Leasing Request whose Machinery has arrived, **When** Company confirms receipt — which is the same act as accepting the machine's condition, its hours reading, the named custodian and the site, all of which `003-deployed-fleet-custody` requires of a handover (its FR-003) — **Then** the confirmation is recorded against that specific Leasing Operation, a Deployment exists for it, and both are retrievable afterward. *(FR-008)*
...
- **AC-012**: **Given** a Lease, **When** Company requests the count and total amount of paid and pending Installments, **Then** the three counts always sum to the Lease's total number of Installments and the three amounts always sum to the Lease's total obligation, with no Installment falling outside all three. *(FR-014)*
