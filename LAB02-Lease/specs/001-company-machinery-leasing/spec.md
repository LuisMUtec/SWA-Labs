...
6. **Given** an Installment whose Certification Milestone has been certified, **When** Company views it, **Then** it is `due`; **and when** Company pays it, **Then** its status becomes `paid` and Company can see it as paid from then on.
...
- **Acquisition Option available but Company has decided neither way**: the Option sits in `available` (undecided) indefinitely; this is a named state, not an error, and `003-deployed-fleet-custody` reads it as "not yet determined" rather than as a Return (FR-021).
...
- **AC-008c**: **Given** an Installment whose Certification Milestone has been certified and whose Leasing Operation has a confirmed Machinery receipt, **When** Company views it, **Then** its status is `due`; **and given** an Installment for which either condition is unmet, **Then** its status is `pending` and Company can retrieve which condition it is still waiting on (BR-04, BR-08). *(FR-010b, FR-010c)*
...
- **AC-012**: **Given** a Lease, **When** Company requests the count and total amount of paid and pending Installments, **Then** the two counts always sum to the Lease's total number of Installments and the two amounts always sum to the Lease's total obligation. *(FR-014)*
...
- **AC-025**: **Given** an Acquisition Option that has been `available` for 30 calendar days without Company exercising or declining it, **When** the window closes, **Then** the Option lapses and the Leasing Operation reaches the same terminal outcome as a decline — closed without acquisition, with the machine returning (BR-11); **and** at any point inside the window Company can retrieve how much of it remains. *(FR-026)*
...
- **AC-023**: **Given** an approved Leasing Request whose conditions require an amount payable before the Installment schedule begins, **When** Company pays it, **Then** the payment is recorded and Company can retrieve that the Installment schedule has begun; **and given** conditions requiring no such amount, **Then** the schedule begins without one. *(FR-024, FR-022)*
...
- **FR-014**: The system MUST allow Company to determine, at any time, both the number and the total amount of `paid` and of `pending` Installments for any of its Leases. **Amended 2026-08-21:** the earlier wording gave counts only, which did not satisfy this feature's own stated need — "Know how much of the Lease's obligation remains" — for an actor whose entire problem is cash flow.
- **FR-015**: The system MUST make the Acquisition Option of a Lease reach `available` when, and only when, every Installment belonging to that Lease is `paid` (BR-07); before that it MUST be `not yet available`. Once `available`, the Option leaves that state only by Company exercising it (FR-017) or declining it (FR-019), never by returning to `not yet available`. **Amended 2026-08-21, per EVAL iteration 01:** the earlier "if and only if … otherwise `not yet available`" contradicted this feature's own four-state model, since after an exercise or a decline every Installment is still paid and the biconditional demanded `available`.
...
- **Acquisition Option**: the right, belonging to a Lease, to acquire its Machinery. Has exactly one status at any time — `not yet available`, `available` (undecided), `exercised`, or `declined`. Availability is determined entirely by whether all of the Lease's Installments are `paid` (BR-07); the choice between `exercised` and `declined` is Company's own act, and it is what determines whether the Leasing Operation ends `Acquired` or `Returned`.
...
9. Company can view the Lease's Installments — each one's status, its amount, and the Certification Milestone of its Project whose certification makes it fall due (BR-04).
10. Each Installment becomes `due` when its Certification Milestone is certified — a step Lease Company performs, staged as step 12 of [`002`](../002-leasing-request-underwriting/spec.md)'s own Stage 1 — and Company has confirmed receipt. **This is the step that makes the POC demonstrate the gap rather than a generic ledger:** payment follows the project's certified progress, not a date.
11. Company pays each Installment once it is `due`.
12. Company can tell, at any point, which Installments are paid, which are due, and which are still pending — and for the pending ones, what they are waiting on.
13. Once every Installment is paid, the Acquisition Option becomes `available`.
14. Company exercises the Acquisition Option.
15. The Leasing Operation reaches an unambiguous completed state.
...
- What forces a decision — exercise or decline — if Company simply lets an available Acquisition Option sit undecided; this feature sets no deadline, and what (if anything) eventually forces the point is unresolved scope shared with `003-deployed-fleet-custody`.
...
- `[ASSUMPTION]` An available Acquisition Option that Company has neither exercised nor declined is a stable, named, non-terminal state (`available`), not a defect. Company may decline explicitly (FR-019), producing `Returned`, symmetric with exercising producing `Acquired`. When a Lease's term ends, and what forces a decision by then if Company does neither, is unresolved scope shared with `003-deployed-fleet-custody` (see its own `[ASSUMPTION]` in Key Product Decisions) rather than something this feature invents unilaterally.
