...
- **AC-006**: **Given** an approved Leasing Request whose Machinery has arrived, **When** Company confirms receipt — which is the same act as accepting the machine's condition, its hours reading, the named custodian and the site, all of which `003-deployed-fleet-custody` requires of a handover (its FR-003) — **Then** the confirmation is recorded against that specific Leasing Operation and is retrievable afterward, carrying the condition, hours, custodian and site Company accepted. *(FR-008)*
