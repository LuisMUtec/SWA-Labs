...
- **AC-024**: **Given** an operation Underwriter determines has defaulted, **When** he records a Default Declaration with its reason, **Then** the declaration exists against that operation, is retrievable with its reason, and is directed to the Fleet Manager responsible for that operation's machine. *(FR-020)*
...
- **AC-032**: **Given** a live operation whose machine has fallen below the balance it still owes, **When** Underwriter retrieves the operation, **Then** that fact is retrievable to him (BR-13). *(FR-029)*
...
- **FR-029**: The system MUST make retrievable, for a live operation, whether its machine has fallen below the balance that operation still owes (BR-13, `003` FR-031), so that erosion of the security behind a live exposure is visible to the Underwriter who decided it. **Added 2026-08-21:** this feature's own Problem states that an approval "puts a machine Lea$e paid for onto a jobsite that may stop paying for it"; the machine's decline as security for that outlay was observable to Fleet Manager and to nobody who carries the exposure.
