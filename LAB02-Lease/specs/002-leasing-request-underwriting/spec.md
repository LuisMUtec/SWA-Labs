...
- **AC-027b**: **Given** a Certification Milestone recorded as certified whose operation's machine the client has confirmed receiving, **When** the instalment anchored to it is evaluated, **Then** it is due; **given** the same milestone with no confirmed receipt, **Then** it is not — an instalment never falls due ahead of delivery (BR-08). *(FR-024)*
