...
- **Project has no Certification Schedule**: the Assessment cannot reach the evidence completeness a Decision requires (FR-010), and no calendar-anchored schedule is offered as a substitute (FR-014).
...
- **AC-008**: **Given** an Assessment with no named Payer, **When** Underwriter attempts to record a Decision, **Then** the system does not record one. *(FR-008, FR-010)*
- **AC-009**: **Given** an Assessment missing any required Evidence Item, **When** Underwriter attempts to record a Decision, **Then** the system does not record one and states which items are missing. *(FR-010)*
...
- **FR-010**: The system MUST NOT allow a Decision to be recorded for an Assessment that is missing any required Evidence Item, and MUST state which items are missing.
