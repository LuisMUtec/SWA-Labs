...
- Be told that a live operation's Project is slipping against those milestones while the contract is still current, and told separately when what secures an operation no longer covers what it owes (BR-13) — a client who pays perfectly can still leave Lea$e under-secured.
- Require a client to restore cover when what secures an operation no longer covers it — and see whether that requirement was met — without ever moving what that client owes.
...
- **How the Authority Limit binds**: it is evaluated against the machinery value of the request. At or below it, Underwriter may record `approved` or `refused`. Above it, approval is not available to him at all — he may record only `escalated` or `refused`. The limit is enforced, not advisory. **Amended 2026-08-21 (EVAL iteration 03):** refusal was previously closed above the limit too, which combined with FR-010's evidence bound to leave an above-limit, under-evidenced request with no available outcome whatsoever. `personas/Carlos.MD` bars him from approving above his limit, not from declining to lend.
...
- **How the Instalment Schedule is anchored**: each instalment is tied to a Certification Milestone of the Project and falls due on the certification of that milestone, not on a calendar date (BR-04). A schedule that cannot be anchored — because the Project has no Certification Schedule — is not a schedule this feature produces. **Resolved 2026-08-21 (EVAL iteration 02):** an instalment falls due on the recording of its milestone's certification, with no offset — an earlier assumption here posited a fixed number of working days afterward, which `001`'s FR-010b and this spec's own AC-027 both contradicted by making the recording itself the trigger. Certification is recorded once the milestone is certified **and paid** (FR-024), so the client is holding the money at that moment and no offset is needed to avoid assuming the two are simultaneous.
...
**Bounded above by value.** Underwriter approves alone at or below his Authority Limit and never above it; above it he prepares and escalates, and may still refuse. The system must make the forbidden outcome — an approval beyond his authority — unavailable rather than merely warn against it.
...
Having approved an operation, Carlos sets its Instalment Schedule against the Certification Milestones of the Project rather than against dates he picked. Each instalment names the milestone whose certification makes it fall due — and as the Project progresses he records each milestone certified, which is what actually makes those instalments payable.
...
**Independent Test**: can be fully tested by taking one approved operation whose Project has a Certification Schedule, producing its Instalment Schedule, confirming every instalment names the Certification Milestone it is anchored to and none carries a bare calendar date, then recording one milestone as certified and confirming that instalment — and only that one — falls due.
...
4. **Given** a Certification Milestone of a live operation's Project, **When** Underwriter records it as certified and paid, **Then** it counts as certified from that point, and the instalment anchored to it falls due once the client has also confirmed receipt.
5. **Given** a milestone Underwriter has not yet recorded as certified, **When** the instalment anchored to it is evaluated, **Then** it has not fallen due — the schedule waits on the project, which is the whole point of anchoring it there.

---

### User Story 3 - Underwriter escalates a request above his authority (Priority: P3)

The request Carlos is holding is for a machine worth more than his Authority Limit. He cannot approve it, and the system does not let him. He prepares the same evidenced Assessment and escalates it, and when the committee resolves it, the resolution lands on that same Assessment rather than in a separate thread.
...
**Independent Test**: can be fully tested by taking one fully evidenced Assessment whose machinery value exceeds the Authority Limit, confirming that approval is unavailable while refusal remains, recording an escalation, and confirming the returned committee resolution attaches to the same Assessment.

**Acceptance Scenarios**:

1. **Given** a fully evidenced Assessment whose machinery value exceeds the Authority Limit, **When** Underwriter attempts to record an approval, **Then** the system does not record one.
...
- **AC-005b**: **Given** an Assessment carrying the machinery value the Applicant stated, **When** Underwriter confirms or corrects it, **Then** the value he recorded is attached to the Assessment as an Evidence Item and is retrievable afterward as the one its Decision was taken against. *(FR-006b)*
- **AC-005c**: **Given** an Assessment with no recorded machinery value, **When** Underwriter attempts to record an approval or an escalation, **Then** the system does not record one; a refusal remains available regardless. *(FR-006b, FR-010)*
...
- **AC-009**: **Given** an Assessment missing any required Evidence Item, **When** Underwriter attempts to record an approval or an escalation, **Then** the system does not record one. *(FR-010)*
- **AC-009b**: **Given** an approval or escalation refused for missing evidence, **When** Underwriter retrieves the refusal, **Then** which Evidence Items are missing is stated. *(FR-010)*
- **AC-009c**: **Given** an Assessment missing any required Evidence Item, **When** Underwriter records a refusal with its reason, **Then** the refusal is recorded — evidence completeness never blocks declining to lend. *(FR-010)*
...
- **AC-010b**: **Given** an approval Underwriter is recording, **When** he sets a down payment above a tenth of the machine's value, **Then** the system rejects it — an upfront demand beyond that recreates the shortfall the client came to Lea$e with (BR-12). *(FR-012)*
...
- **AC-012**: **Given** an Assessment whose machinery value exceeds the Authority Limit, **When** Underwriter attempts to record `approved`, **Then** the system does not record it. *(FR-011)*
- **AC-012b**: **Given** an Assessment whose machinery value exceeds the Authority Limit and whose evidence is incomplete, **When** Underwriter records a refusal with its reason, **Then** the refusal is recorded — deciding not to lend is his at any value, and it is never blocked for want of evidence. *(FR-011, FR-010)*
...
- **AC-014d**: **Given** an approval whose Conditions include a guarantee due before the operation proceeds, **When** Underwriter records that guarantee as in place, **Then** the Condition is met, it no longer appears among the operation's outstanding Conditions, and the operation may proceed past the point it governed. *(FR-012c, FR-012b)*
- **AC-014b**: **Given** an escalated Assessment whose committee resolution approves the operation with its Conditions, **When** it is recorded, **Then** the Leasing Request becomes `approved` and an Instalment Schedule anchored to the Project's Certification Milestones is produced, exactly as for an approval within the Authority Limit (BR-04). *(FR-022, FR-014)*
- **AC-014c**: **Given** an approval whose Conditions include an amount or a guarantee due before the operation proceeds, **When** those Conditions have not been recorded as met, **Then** the Installment schedule has not begun. *(FR-012b)*
...
- **AC-016**: **Given** an approved operation whose Project has a Certification Schedule, **When** its Instalment Schedule is produced, **Then** every instalment names the Certification Milestone whose certification makes it fall due (BR-04). *(FR-014)*
- **AC-016c**: **Given** a produced Instalment Schedule, **When** its instalments are examined, **Then** none falls due on a fixed calendar date — the schedule is anchored to the project's certified progress, never to dates Lea$e chose (BR-04). *(FR-014)*
...
- **AC-021**: **Given** a Slippage Warning, **When** it is raised, **Then** it is directed to the Underwriter who recorded that operation's Decision. *(FR-017)*
- **AC-021b**: **Given** a Slippage Warning being raised, **When** the operation's state is examined, **Then** its machine has neither been returned, nor retired to the client by acquisition, nor recovered, and no Default Declaration stands against it — the warning reaches Underwriter while the operation is still current. *(FR-017, FR-019)*
...
- **AC-024b**: **Given** a recorded Default Declaration, **When** it is raised, **Then** it is directed to the Fleet Manager responsible for that operation's machine. *(FR-020)*
...
- **AC-026**: **Given** a live operation with either one Certification Milestone more than 30 calendar days past its expected date and still uncertified, or two Certification Milestones of the same Project simultaneously past their expected dates and still uncertified, **When** Underwriter retrieves the operation, **Then** it is shown as eligible for a Default Declaration (BR-09). *(FR-023)*
- **AC-026b**: **Given** an operation that has reached Default-Declaration eligibility, **When** no Underwriter has recorded a declaration against it, **Then** none exists — the threshold never declares a default by itself. *(FR-023)*
- **AC-031**: **Given** an Assessment carrying a request for further evidence, **When** the Applicant supplies what was asked for, **Then** the response is retrievable against that Assessment and Underwriter is able to see it. *(FR-028)*
- **AC-032**: **Given** a live operation whose cover has fallen below the balance it still owes, **When** Underwriter retrieves the operation, **Then** it is shown as impaired (BR-13). *(FR-029)*
- **AC-032d**: **Given** an operation shown as impaired, **When** Underwriter records a requirement that the client restore cover, **Then** it is retrievable with its reason, and nothing the client owes and no date on which it falls due has changed (BR-13). *(FR-030, FR-015)*
- **AC-032e**: **Given** a requirement to restore cover recorded against an operation, **When** Company retrieves that operation, **Then** what is being asked for and why is stated to it (`001` FR-029). *(FR-030)*
- **AC-032f**: **Given** an impaired operation whose shortfall is a stated amount, **When** Underwriter attempts to require cover beyond that shortfall, **Then** the system does not record the requirement — a demand larger than the gap it closes recreates the shortfall the client came to Lea$e with (BR-13). *(FR-030)*
- **AC-032g**: **Given** an outstanding restore-cover requirement, **When** Underwriter records what the client supplied as Additional Security, **Then** the requirement is retrievable as met. *(FR-030b)*
- **AC-032h**: **Given** an impaired operation, **When** Additional Security is recorded that brings its cover to at least its outstanding balance, **Then** the operation is no longer retrievable as impaired (BR-13). *(FR-029, FR-030b)*
- **AC-032b**: **Given** a live operation whose cover falls below its outstanding balance for the first time, **When** that becomes true, **Then** it is directed to the Underwriter who recorded that operation's Decision. *(FR-029)*
- **AC-032c**: **Given** an operation shown as impaired, **When** its Project's milestones are all certified on time and no instalment is unpaid, **Then** it is still impaired — impairment turns on the security, not on the client's payment behaviour. *(FR-029)*
- **AC-029**: **Given** a Payer that has been recorded on earlier Assessments and whose Projects raised Slippage Warnings, **When** Underwriter records that same Payer on a new Assessment, **Then** those warnings and whether each slipped milestone was later certified are retrievable with it. *(FR-026)*
- **AC-030**: **Given** Leasing Requests awaiting a Decision, **When** Underwriter retrieves his queue, **Then** each carries how long it has been waiting. *(FR-027)*
...
- **AC-027**: **Given** a Certification Milestone, **When** Underwriter records it as certified and paid, **Then** it is no longer counted as uncertified by any Slippage Warning evaluation. *(FR-024)*
- **AC-027b**: **Given** a Certification Milestone recorded as certified whose operation's machine the client has confirmed receiving, **When** the instalment anchored to it is evaluated, **Then** it is due. *(FR-024)*
- **AC-027c**: **Given** a Certification Milestone recorded as certified whose operation's machine the client has not confirmed receiving, **When** the instalment anchored to it is evaluated, **Then** it is not due — an instalment never falls due ahead of delivery (BR-08). *(FR-024)*
...
- **FR-006b**: The system MUST require the machinery's value to be recorded as an Evidence Item of every Assessment — stated by the Applicant at submission (`001` FR-002) and confirmed or corrected by Underwriter — and MUST NOT record `approved` or `escalated` for an Assessment that lacks it — refusal stays available, as it does for every other missing Evidence Item (FR-010). **Added 2026-08-21, per EVAL iteration 06:** FR-011's Authority Limit, BR-12's down-payment cap (AC-010b) and Stage 1 step 8 all turn on "the request's machinery value", and no requirement made anyone record it — so the one bound `personas/Carlos.MD` states as absolute rested on a figure whose origin the specification did not name. This is the same defect twice repaired elsewhere in this set, at FR-015's "signed" schedule and `003` FR-031b's Assessed Value. **Amended 2026-08-21, per EVAL iteration 07:** as first written this barred *every* Decision, refusal included, contradicting FR-010, AC-008, AC-009c and AC-012b outright and re-creating the exact state FR-011's iteration-03 amendment exists to prevent — an Assessment with no recorded machinery value had no available outcome at all: approval and escalation barred, refusal barred too, and the Authority Limit not even evaluable. `personas/Carlos.MD`: "My only fast answer today is 'no'." A required Evidence Item bounds lending; it never bounds declining to lend.
...
- **FR-011**: The system MUST evaluate the request's machinery value against the Authority Limit, MUST allow `approved` only at or below it, and above it MUST allow only `escalated` or `refused`. **Amended 2026-08-21, per EVAL iteration 03:** refusal was previously closed above the limit as well, and FR-010 also closes escalation for an Assessment missing required evidence — so an above-limit request with an unnamed Payer or no Certification Schedule had *no* available outcome at all, and AC-009's guarantee that "a refusal remains available regardless" contradicted AC-012. `personas/Carlos.MD` bars Underwriter from approving above his limit; it does not bar him from refusing, and the fast no is the one instrument his file names as the thing he cannot afford to lose.
- **FR-012**: The system MUST allow Underwriter to record an approval carrying its reason and its Conditions — down payment, term, guarantees, and which machine — MUST NOT allow an approval without Conditions, and MUST NOT allow a down payment exceeding 10% of the machine's value (BR-12).
- **FR-012c**: The system MUST allow each Condition of an approval to be recorded as met, whatever its kind — an amount by Company paying it (`001` FR-024), a guarantee or any other non-monetary term by Underwriter recording that it is in place — and MUST make the outstanding Conditions of an operation retrievable at any time. **Added 2026-08-21, per EVAL iteration 03:** FR-012b gates the operation on Conditions being "recorded as met" and the only act of satisfaction anywhere was paying an amount, so an approval carrying a guarantee — one of the four Condition kinds this feature names as Underwriter's to set — stalled at a gate with no key. Since FR-012 forbids an approval without Conditions, that is the ordinary conditional approval, which is the instrument by which Underwriter says yes to a case he would otherwise refuse.
- **FR-012b**: The system MUST record, for each Condition of an approval, whether it must be satisfied before the operation proceeds, and MUST NOT allow the operation to proceed past the point each such Condition governs until it is recorded as met — the Installment schedule does not begin before the amounts and guarantees due beforehand are in place (`001` FR-024), and no machine is handed over that is not the machine the Conditions name (`003` FR-001b). **Added 2026-08-21, per EVAL iteration 02:** Conditions were recorded and reviewable and gated nothing. An approval granted on a down payment and a guarantee could reach a jobsite with neither in place, which makes the conditional approval — the instrument by which Underwriter says yes to a case he would otherwise refuse — narrower in force than it reads.
...
- **FR-020**: The system MUST allow Underwriter to record a Default Declaration with its reason against an operation, MUST direct it to the Fleet Manager responsible for that operation's machine, and that declaration MUST authorise recovery without performing it. **Amended 2026-08-21, per EVAL iteration 02:** FR-017 directs a Slippage Warning to the Underwriter who decided the operation, and the Default Declaration — the event that actually requires someone else to act — had no matching clause. Fleet Manager could act on a declaration once she knew of it (`003` FR-022) and no requirement said she was told.
...
- **FR-022**: The system MUST allow an escalated Assessment's committee resolution to be recorded against that same Assessment, retrievable with it, and MUST give a resolution that approves the operation the same effect as an approval recorded by Underwriter — it carries its Conditions, makes the Leasing Request `approved` in `001`'s terms, and produces an Instalment Schedule anchored under FR-014. **Amended 2026-08-21, per EVAL iteration 02:** a favourable committee resolution previously returned onto the Assessment and stopped there, since only "an approved Decision with its Conditions" made a request `approved` and FR-014 produced a schedule only for "an approved operation" — so every operation above the Authority Limit came back from the committee with nowhere to go.
...
- **FR-028**: The system MUST make an Applicant's response to a request for further evidence (`001` FR-025) retrievable against the Assessment it was raised on, and MUST make it observable to the Underwriter who raised it.
- **FR-030**: The system MUST allow Underwriter to require, against an impaired operation, that the client restore cover — by additional security, or by returning the machine's condition above the line — recorded with its reason and retrievable afterward, MUST make that requirement observable to the client it is made of (`001` FR-029), MUST NOT allow it to demand more than the shortfall it answers, and MUST NOT let it alter what the client owes or when it falls due (FR-015). **Added 2026-08-21, per EVAL iteration 06:** impairment reached Underwriter as a label he could act on in no way, which made BR-13 a sort key rather than a rule. This is the act it unlocks, and it is the only one consistent with `personas/Carlos.MD`: he may set the conditions of an operation and may never move a schedule, so restoring security is his lever and rescheduling is not. `[ASSUMPTION: that Lea$e may ask for security to be restored mid-contract is ours; the brief fixes no remedy. What the client must do if he will not, and whether refusal is itself a default ground, is deliberately left to a later stage rather than invented here.]` **Amended 2026-08-21, per EVAL iteration 07:** the requirement commanded a client who could not see it, could not be recorded as having met it, and was bounded by nothing — three ways of being an act in name only. Its bound is the shortfall itself, for BR-12's reason: an upfront demand beyond what closes the gap recreates, mid-contract, the shortfall the client came to Lea$e with. Its path to the client follows this feature's own precedent at FR-025, answered by `001` FR-025.
- **FR-030b**: The system MUST allow what the client supplies against a restore-cover requirement (FR-030) to be recorded as Additional Security of that operation, and MUST make that requirement retrievable as either outstanding or met — met only by what is recorded, and by nothing else. **Added 2026-08-21, per EVAL iteration 07:** FR-030 could be raised and never closed, which is the stalled-gate defect FR-012c was added in iteration 03 to remove — "an approval carrying a guarantee stalled at a gate with no key" — reappearing one state later.
- **FR-029**: The system MUST make retrievable, for a live operation, whether its **cover** — the machine's Assessed Value (`003` FR-031b) plus any Additional Security recorded against that operation (FR-030b) — has fallen below the balance that operation still owes (BR-13, `003` FR-031), MUST direct that fact to the Underwriter who recorded the operation's Decision when it first becomes true, MUST record the operation as impaired from that point — a state distinct from, and reachable without, the slippage of FR-017 or the default eligibility of FR-023 — and MUST record it as no longer impaired once its cover stands at or above that balance again. **Amended 2026-08-21, per EVAL iteration 05:** the fact was visible and wired into nothing, so it changed no decision anywhere. An impaired operation is one whose security no longer covers it while the client is still paying on time; that is a risk event with no equivalent in a business that rents, and it reaches the person carrying the exposure rather than sitting in a fleet report. **Added 2026-08-21:** this feature's own Problem states that an approval "puts a machine Lea$e paid for onto a jobsite that may stop paying for it"; the machine's decline as security for that outlay was observable to Fleet Manager and to nobody who carries the exposure. **Amended 2026-08-21, per EVAL iteration 07:** impairment was measured on the machine alone, so of FR-030's two named remedies only one could ever end it — a client who posted additional security left the operation impaired for good, which made the demand unanswerable by construction and left the state with no exit. Cover is the measure BR-13's own Consequence assumes. `003` FR-031 goes on comparing the machine alone, deliberately: a guarantee posted to a lender does not repair a worn machine, so Fleet Manager orders service by the machine and Underwriter carries the exposure by the cover.
- **FR-026**: The system MUST make retrievable, when Underwriter records a Payer on an Assessment, that Payer's history across Lea$e's other operations — the Slippage Warnings raised on Projects it was paying and whether each slipped milestone was later certified. **Added 2026-08-21, per EVAL iteration 01:** AC-023 keeps a Slippage Warning retrievable on the stated ground that "the record of a Project having slipped is itself evidence for the next decision", and no requirement delivered that evidence to the next decision. A Payer pays many applicants; this is the one report on a Payer that Lea$e can actually produce, and withholding it left Underwriter opening each new Assessment with an empty behaviour field on the risk he says he cares most about.
- **FR-027**: The system MUST make retrievable, for every Leasing Request awaiting a Decision, how long it has been waiting, so that Underwriter can order his queue by age rather than by arrival. `[ASSUMPTION: this makes the wait visible on the deciding side, matching what `001`'s FR-023 makes visible on the applicant's side. It commits Lea$e to no turnaround — that stays a business decision — but "speed is a decision too" (personas/Carlos.MD) cannot be acted on at all while the queue is undifferentiated.]`
...
- **Evidence Item**: one recorded piece of support attached to an Assessment — Applicant eligibility, Credit Standing, machinery value, Project identification, Certification Schedule, Payer. Required set is the same for every Assessment.
...
- **Additional Security**: anything recorded against an operation, beyond the machine itself, that stands behind its outstanding balance. With the machine's Assessed Value it forms the operation's **cover** (FR-029), and it is what a restore-cover requirement is discharged by (FR-030b).
...
**Requirements this feature defines that Stage 1 does *not* exercise**, so Principle V's boundary is applicable to the whole set: FR-025 and FR-028 (requesting further evidence and receiving the answer — Stage 1's Assessment is complete when taken up), FR-026 (a Payer's history across operations — Stage 1's Payer is new to Lea$e), FR-027 (queue age — Stage 1 has one request), FR-029, FR-030 and FR-030b (impairment, the requirement to restore cover, and its discharge — Stage 1's machine is new and its balance falls faster than its value, so no operation becomes impaired and no cover needs restoring), FR-013 (refusal), FR-011's escalation branch with FR-022, and FR-017–FR-021, FR-023 (slippage, eligibility, default). Each is specified here and demonstrated later. Stage 1 **does** exercise certification, at step 13, which `001`'s Stage 1 step 11 depends on by name; those milestones are certified on time in the happy path, and a milestone passing its expected date uncertified is the Slippage Warning of User Story 4, which is later-stage.
...
- **SC-003**: No `approved` Decision exists for a request whose machinery value exceeds the Authority Limit.
...
- **SC-005**: For a Project whose milestone passes its expected date uncertified, the warning reaches the deciding Underwriter while the operation is still open — its machine neither returned, nor retired by acquisition, nor recovered, and no Default Declaration standing against it — and before any instalment is overdue.
- **SC-006**: Every Assessment presents the same required Evidence Items regardless of its machinery value, so any difference between two outcomes is attributable to their content.
