...
- Be told that a live operation's Project is slipping against those milestones while the contract is still current, and told separately when the machine securing an operation no longer covers what it owes (BR-13) — a client who pays perfectly can still leave Lea$e under-secured.
- Require a client to restore cover when the machine securing an operation no longer covers it, without ever moving what that client owes.
...
- **AC-005b**: **Given** an Assessment carrying the machinery value the Applicant stated, **When** Underwriter confirms or corrects it, **Then** the recorded value is what FR-011's Authority Limit and BR-12's cap are evaluated against. *(FR-006b, FR-011)*
- **AC-005c**: **Given** an Assessment with no recorded machinery value, **When** Underwriter attempts to record any Decision, **Then** the system does not record one. *(FR-006b)*
...
- **AC-032**: **Given** a live operation whose machine has fallen below the balance it still owes, **When** Underwriter retrieves the operation, **Then** it is shown as impaired (BR-13). *(FR-029)*
...
- **AC-032b**: **Given** a live operation whose machine falls below its outstanding balance for the first time, **When** that becomes true, **Then** it is directed to the Underwriter who recorded that operation's Decision. *(FR-029)*
...
- **FR-030**: The system MUST allow Underwriter to require, against an impaired operation, that the client restore cover — by additional security, or by returning the machine's condition above the line — recorded with its reason and retrievable afterward, and MUST NOT let that requirement alter what the client owes or when it falls due (FR-015). **Added 2026-08-21, per EVAL iteration 06:** impairment reached Underwriter as a label he could act on in no way, which made BR-13 a sort key rather than a rule. This is the act it unlocks, and it is the only one consistent with `personas/Carlos.MD`: he may set the conditions of an operation and may never move a schedule, so restoring security is his lever and rescheduling is not. `[ASSUMPTION: that Lea$e may ask for security to be restored mid-contract is ours; the brief fixes no remedy. What the client must do if he will not, and whether refusal is itself a default ground, is deliberately left to a later stage rather than invented here.]`
- **FR-029**: The system MUST make retrievable, for a live operation, whether its machine has fallen below the balance that operation still owes (BR-13, `003` FR-031), MUST direct that fact to the Underwriter who recorded the operation's Decision when it first becomes true, and MUST record the operation as impaired from that point — a state distinct from, and reachable without, the slippage of FR-017 or the default eligibility of FR-023. **Amended 2026-08-21, per EVAL iteration 05:** the fact was visible and wired into nothing, so it changed no decision anywhere. An impaired operation is one whose security no longer covers it while the client is still paying on time; that is a risk event with no equivalent in a business that rents, and it reaches the person carrying the exposure rather than sitting in a fleet report. **Added 2026-08-21:** this feature's own Problem states that an approval "puts a machine Lea$e paid for onto a jobsite that may stop paying for it"; the machine's decline as security for that outlay was observable to Fleet Manager and to nobody who carries the exposure.
