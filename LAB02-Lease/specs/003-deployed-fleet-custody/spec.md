...
- Transport contracting and route planning for a recovery. What a Recovery carries is fixed by FR-022 — the machine's last known location, its last hours reading, its Custodian and its Contracted Site — so the work starts from what Lea$e already knows rather than from nothing. Arranging the transport, the access and the route themselves is not this feature, and neither is recording them. `[ASSUMPTION: an earlier draft of this section claimed transport, access and a route were recorded; no requirement did so, and the claim is withdrawn here rather than turned into scope, because arranging a recovery is field logistics rather than a fact about the fleet.]`
...
- **How a Deployment closes**: by exactly one of Return or Acquisition Retirement, never both and never neither, once the operation's Acquisition Option has been exercised or declined (BR-07) — which is `001`'s behaviour, not this feature's. **Amended 2026-08-21:** while the Option is merely `available` (neither exercised nor declined), a live Deployment does not close at all, by either end — Fleet Manager sees "not yet determined" (FR-016) instead of a premature Return. What this feature adds is that Fleet Manager can see, at any point, which of the three states applies, before the term ends. `[ASSUMPTION: what marks "the term ending" — the point by which Company must have exercised or declined, if anything forces it at all — is shared, unresolved scope with 001-company-machinery-leasing, not a gap unique to either. Both specs fix what determines the outcome once Company acts; neither fixes what, if anything, forces Company to act. Stage 1 does not need this resolved: its own scenario chooses the closing moment directly.]`
