...
- **Fleet Manager attempts to stop a machine over an unpaid installment**: no capability of this feature allows it, whether or not she believes the client has stopped paying (FR-019, FR-021).
...
- **AC-020**: **Given** a Deployment whose client has acquired the machine, **When** it closes, **Then** the machine is retired from the fleet — the client's right to acquire follows from having paid every installment (BR-07). *(FR-017)*
...
- **AC-024**: **Given** a live Deployment with no declared default, **When** Fleet Manager attempts to stop the machine citing a cause that is not a risk to people or to the machine — to force a service window, to settle a dispute, or over an unpaid installment — **Then** the system does not allow it. *(FR-019)*
...
- **AC-038c**: **Given** an Incident being recorded against a live Deployment, **When** it is recorded, **Then** whether it stops the machine working is recorded with it. *(FR-013)*
- **AC-038f**: **Given** an Incident recorded as stopping work, **When** it is recorded, **Then** an Unavailability is open against its Deployment. *(FR-032, FR-013)*
- **AC-038g**: **Given** an Incident recorded as stopping work, **When** Fleet Manager records it as no longer stopping work, **Then** the Unavailability it opened is closed. *(FR-032, FR-013)*
- **AC-038h**: **Given** a machine under a Safety Stop, **When** Fleet Manager records the cited cause as resolved, **Then** the stop is lifted and the Unavailability it opened is closed. *(FR-032, FR-019)*
- **AC-038i**: **Given** an agreed Service Window that passes with the service outstanding, **When** the Deployment is retrieved, **Then** no Unavailability is open on that ground alone — a postponed service does not stop the machine. *(FR-032, FR-012)*
- **AC-038d**: **Given** a closed Unavailability, **When** it is retrieved, **Then** how long it ran is retrievable. *(FR-032)*
...
- **AC-033**: **Given** two live Deployments, one whose operation has every installment but the last paid and one on its third of twenty-four, **When** Fleet Manager retrieves her deployed machines, **Then** each carries its installment progress, so the two are distinguishable before either reaches its end. *(FR-028)*
...
- **FR-013**: The system MUST allow Fleet Manager to record an Incident against a live Deployment, retrievable afterward with when it was recorded and what was known then, MUST require every Incident to carry whether it stops the machine working, and MUST allow one that does to be later recorded as no longer stopping work. An Incident recorded as stopping work opens an Unavailability (FR-032), which that later record closes. **Amended 2026-08-21, per EVAL iteration 09:** FR-032 was written to trigger on "an Incident recorded as stopping work" and no requirement gave an Incident that attribute, so AC-038c could be declared neither met nor unmet, and nothing could ever end what it opened.
...
- **FR-019**: The system MUST allow Fleet Manager to record a Safety Stop against a live Deployment, citing as its cause a risk to the physical safety of people or of the machine itself — a recorded Incident of that kind (FR-013), a finding of an Inspection (FR-026), or a machine overdue for service past BR-10's threshold (FR-025). The system MUST allow her to record the cited cause as resolved, which lifts the stop, and MUST NOT allow her to stop a machine on a cause outside that class unless a Default Declaration exists for its operation. A Safety Stop opens an Unavailability (FR-032) that runs until it is lifted. **Amended 2026-08-21, per EVAL iteration 09:** nothing in this feature ever ended a Safety Stop. Once FR-032 hung a record on it, one exercise of the single unconditional permission `personas/Julia.MD` grants her would have opened an Unavailability that never closed — and `002` FR-031 would then have answered "Lea$e's machine was standing" for every later milestone of that project, in the hands of the person who decides defaults, with no act available to her to retract it. An un-retractable cost attached to a safety stop makes safety the most expensive act in the system, which is the opposite of what her file states. **Amended 2026-08-21, per EVAL iteration 01:** "a cause that is not safety" was previously undefined while carrying the whole weight of AC-024, so no one could declare that criterion met or unmet. `[ASSUMPTION: the class is stated by what it protects — people or the machine — rather than by an exhaustive list, because a list would be wrong the first time the field produced a hazard nobody enumerated. What it excludes is what personas/Julia.MD forbids her: stopping a machine to force a service window, to settle a dispute, or to apply pressure over a payment.]`
...
- **FR-032**: The system MUST open an **Unavailability** against a Deployment whenever its machine stops being available to work — a Safety Stop (FR-019) or an Incident recorded as stopping work (FR-013) — MUST close it when that stop is lifted or that Incident is recorded as no longer stopping work, and MUST make retrievable, for each one, how long it ran and which Certification Milestones of the operation's Project had their expected date fall inside it, those dates being the ones the Project's Certification Schedule carries (`002` FR-007). **Amended 2026-08-21, per EVAL iteration 09:** as first written this had three causes and no terminator for any of them. A missed Service Window was one of the three, and this feature's own Key Product Decisions hold that below BR-10's threshold nothing forces the machine to stop and a client who postpones is in breach of nothing — so the record contradicted its own definition, and `002` FR-031 would have reported a machine that ran two shifts a day as having been standing. That trigger is removed rather than reworded: a postponed service is a service that did not happen, not a machine that stopped. **Added 2026-08-21, per EVAL iteration 08:** the aggregator's standing D2 finding is that most of this feature's body would survive the financing gap disappearing, and it named FR-012, FR-013 and FR-019 among them. This is what makes those three Lea$e's requirements rather than a rental firm's. This spec's Problem has argued it in prose since iteration 04 and never made it a requirement: *a machine standing idle for want of a service does not cost Lea$e a day's rent — it stops the progress that certifies, and an uncertified milestone is an installment that does not fall due* (`001` FR-010b, BR-04). A rental firm records downtime to credit rental days against an invoice it would otherwise raise; Lea$e records it because the event its own repayment depends on may not happen. Same fact, opposite consequence, and only one of the two has any reason to know which milestone was standing underneath.
...
- **FR-028**: The system MUST make retrievable, for every live Deployment, how far its operation's installments have progressed — how many are paid and how many remain. **Added 2026-08-21, per EVAL iteration 01:** FR-016 tells Fleet Manager which end a Deployment is heading for, but that answer is "not yet determined" until the last installment is paid and Company acts, so it arrives too late to plan the next contract around. `personas/Julia.MD` names exactly this: "I plan the next contract around a machine that may never come back, and nothing tells me which ones those are until it is done." Installment progress is the forward signal — a machine one payment from acquisition is visibly different from one on its third of twenty-four — and it is a fact `001` already holds (its FR-014). `[ASSUMPTION: this gives Fleet Manager a count of her own operation's progress, not a view into the client's finances; `001`'s FR-018 scopes Company's own data to Company, and this is Lea$e reading the state of its own asset's contract.]`
...
- **Unavailability**: a recorded period during which a deployed machine was not available to work — a Safety Stop, or an Incident that stops it — carrying its cause, its length once closed, and the Certification Milestones whose expected date fell inside it (FR-032).
...
- **Incident**: a recorded event affecting a deployed machine, with when it was recorded, what was known then, and whether it stops the machine working (FR-013).
...
- A client who has paid every installment may acquire the machine — this restates BR-07, not an invented assumption.
