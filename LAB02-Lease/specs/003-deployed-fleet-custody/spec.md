...
- **How a Deployment closes**: by exactly one of Return or Acquisition Retirement, never both and never neither. Which one applies is determined by whether the operation's Acquisition Option was exercised (BR-07), which is `001`'s behaviour, not this feature's. What this feature adds is that Fleet Manager can see which end a live Deployment is heading for before the term ends. `[ASSUMPTION: what marks "the term ending" — the moment Close is triggered — is shared, unresolved scope with 001-company-machinery-leasing, not a gap unique to either. Both specs fix what determines the outcome once closing happens (exercised vs. not); neither fixes when closing happens. Stage 1 does not need this resolved: its own scenario chooses the closing moment directly.]`
