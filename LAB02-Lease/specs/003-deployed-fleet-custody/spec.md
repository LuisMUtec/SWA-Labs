...
**How much of this feature is gap-specific, honestly.** Custody, wear and servicing are the concerns of any machinery business, and a fair reading of FR-003 through FR-014 finds requirements a rental fleet would need too. This feature does not claim otherwise. What it claims is narrower and checkable: three things here exist *only* because Lea$e finances rather than rents, and they are what the rest is in service of.

*The obligation runs the wrong way for a rental.* Lea$e is repaid out of the client's project, and only as that project is certified (BR-04). A machine standing idle for want of a service does not cost Lea$e a day's rent — it stops the progress that certifies, and an uncertified milestone is an instalment that does not fall due (`001` FR-010b). A rental firm's downed machine costs it revenue directly; Lea$e's costs it the event its own repayment depends on. That is why the hours clock (BR-06), the service window and BR-10's threshold sit in a leasing company at all, and it is why FR-028 exposes instalment progress to a fleet manager who in a rental business would have no business seeing it.

*Two of the three ends are financing events.* Acquisition Retirement happens only because instalments finished (BR-07); Recovery Close only because they stopped. Return — the third — is indeed the ordinary end a rental would also have, and this feature does not pretend it is anything else.
...
- **What a handover requires**: a condition record, an Operating-Hours Reading, a named Custodian, a Contracted Site, and the Custodian's acceptance of the obligation to report a departure from that site (FR-014b) — all five, accepted by both sides. A handover missing any of them is not a handover this feature performs, because the whole value of the record is that it was agreed before there was anything to argue about.
...
1. **Given** a live Deployment, **When** Fleet Manager asks which end it is heading for, **Then** the answer is Acquisition Retirement if the Option has been exercised, Return if it has been declined, or "not yet determined" if it is still simply `available` (BR-07).
...
- **AC-020**: **Given** a Deployment whose client has acquired the machine, **When** it closes, **Then** the machine is retired from the fleet and its condition and hours at retirement are settled against the Handover Record — the client's right to acquire follows from having paid every instalment and is unaffected by what that settlement records (BR-07). *(FR-017)*
- **AC-020b**: **Given** a machine, **When** Fleet Manager sets its Service Interval, **Then** that interval governs its Service Due determination, and continues to when the machine moves to a later Deployment (BR-06). *(FR-017b, FR-007)*
...
- **AC-022b**: **Given** a Deployment whose machine came back under a Recovery, **When** it is closed by Recovery Close, **Then** the machine's condition and hours at recovery are settled against its Handover Record, and the machine is no longer blocked from a later Deployment. *(FR-018b, FR-002)*
...
- **AC-035**: **Given** a live Deployment, **When** what its recovery would require — transport, access and route — is recorded against it, **Then** that record is retrievable, and any Recovery later opened for that Deployment carries it. *(FR-030, FR-022)*
...
- **FR-003**: The system MUST require a handover to carry the machine's condition, an Operating-Hours Reading, a named Custodian, a Contracted Site, and the Custodian's acceptance of the departure-reporting obligation of FR-014b, each accepted by both Lea$e and the client, and MUST NOT open a Deployment when any is missing.
...
- **Handover Record**: condition, Operating-Hours Reading, Custodian, Contracted Site and the Custodian's departure-reporting obligation at handover, accepted by both sides. Immutable; supersedable.
...
2. Fleet Manager records the handover — condition, hours, named Custodian, Contracted Site, and the Custodian's departure-reporting obligation — and both sides accept.
