...
- **Consequence:** a machine below the line is serviced before machines above it, its operation is treated as impaired, and Lea$e may require the client to restore cover — by additional security, or by bringing the machine's condition back above the line. What Lea$e may not do is move what the client owes or when: the shortfall is Lea$e's to address, not the client's to pay for early.
