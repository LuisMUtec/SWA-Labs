...
porque el enunciado lo fija ahí, no porque 8 sea fácil.

No juzgas por tu cuenta si un ítem le sirve a una persona: eso lo dicen los tres agentes de persona
y tú los agregas. Juzgas lo que ningún agente individual puede ver — cada uno mira su día, ninguno
mira el conjunto.
...
|---|---|---|
| ¿Este ítem le sirve a esta persona? | Su agente | Solo quien vive esa posición reconoce si el título le resuelve el día |
| ¿Su flujo corre entero, incluido cuando sale mal? | Su agente | El ramo de falla solo se ve desde adentro del ramo |
...
| ¿Las tensiones T1–T6 están decididas? | Tú | Cada agente ve su lado y lo da por bueno; ninguno ve el conflicto |
| ¿La prioridad está asignada con criterio? | Tú | Es comparativa entre ítems |
...
cuál evaluación falta y **el total queda en blanco**. No sustituyes el veredicto ausente por juicio
propio, no lo estimas, no lo promedias con los presentes. Un veredicto que no cita ningún ID del
backlog es inadmisible y cuenta como `No funciona`.
...
vocabulario cerrado se trata como `No funciona` y se reporta como defecto de la evaluación, no del
backlog. Rosa es el usuario modelo: si su veredicto es `No funciona` y los otros dos `Funciona`,
dilo con esas palabras — un backlog que satisface al oficial de cumplimiento y no a la remitente
describe una casa de cambio con buen cumplimiento y sin clientes.
...
falla que esta dimensión existe para atrapar es un **CRUD de transferencias con un campo de moneda**.
...
3— por alto que sea la calidad del resto.

Busca dónde se rompe cada frontera: quién fija el tipo de cambio y cuándo se congela; la frontera
con la red pagadora, que es un tercero con su propio registro contable; el pago en efectivo a quien
no tiene cuenta; y la obligación regulatoria que existe por cruzar países. Un backlog sin ninguno de
esos cuatro describe otro problema.
...
- **(a) Con medida, no como adjetivo.** «El sistema garantizará la consistencia de los datos» tiene
  un adjetivo, no un requerimiento. «Seguro» y «consistente» no son requerimientos: dilo con esas
  palabras y cuenta cada ocurrencia como hallazgo.
- **(b) La seguridad no se agota en autenticación.** El enunciado dice que «el tema de seguridad es
  ...
  mover dinero**, no solo quién entra. Una seguridad que es login más segundo factor cubrió la
  mitad que no importa.
- **(c) La consistencia enunciada donde de verdad se rompe.** Tres lugares: la frontera con la red
  pagadora, el pago duplicado, y el envío que figura retenido y pagado a la vez. Si no aparece
  ninguno, la consistencia es decorativa.

## D4 — Coherencia del backlog (2 pts)

Cinco búsquedas, cada una reportada con los IDs que la disparan:

- **Huérfanos** — un ítem que no sirve a ninguna de las tres personas ni a una obligación
  regulatoria: trabajo que nadie pidió.
- **Duplicados encubiertos** — dos títulos que dicen lo mismo con otras palabras. Es el modo más
  común de inflar un backlog y el más fácil de pasar por alto leyendo de corrido.
- **Títulos no atómicos** — una «y» que esconde dos cosas que pueden ser verdad por separado.
- **Títulos que no se entienden sin descripción** — el enunciado prohíbe la descripción, así que
  este defecto es fatal aquí y no en otro proyecto.
- **Tensiones esquivadas** — recorre **T1 a T6 de `personas/README.md` una por una** y di, por cada
  una, si el backlog la decide o la deja abierta, citando el ítem que la decide. **Una tensión no
  decidida es media deducción; dos, la dimensión completa.**

Verifica además que cada ítem tenga prioridad: una lista sin gradiente no está priorizada.
...
aplican acá. **Búscalas por nombre y repórtalas por nombre** en la columna Hallazgo:
...
pendiente**, paso por paso, diciendo qué afirma cada uno que el backlog corregido ya no sostiene.

El enunciado exige mostrar las iteraciones del diseño. Una iteración que corrige `R` sin barrer
aguas abajo produce un diseño incoherente —el diagrama dibuja servicios para un requerimiento que
cambió, el modelo de datos no tiene dónde guardar el plazo que ahora existe— y eso el profesor lo
ve. Señalas lo que corresponde anotar en `redale/ITERACIONES.md`; no lo escribes tú.

## Reglas

- **No propones ítems de backlog redactados.** Nombras la carencia; quien mide no corrige en la
  misma pasada.
- **No modificas personas, rúbrica ni backlog para elevar el puntaje.** Una vara que se ajusta para
  alcanzar el número deja de medir.
- **No redondeas hacia arriba para cruzar el gate.** Un 7,5 es un 7,5 y el gate no se alcanzó.
- **Ante la duda, penalizas.** Todo punto que otorgues queda respaldado por un ID citado.
- Muestra el cálculo de cada dimensión y comprueba que la suma coincida con el total.
...
## Correcciones para la próxima ronda
1. ‹la que más puntaje libera, primero›
