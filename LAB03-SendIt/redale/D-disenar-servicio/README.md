...
| `RF-12` — «el sistema comprobará la respuesta del desafío sin mostrarla a quien atiende el mostrador», con el invariante de `RNF-17` **citado como está**: «El sistema puede comprobar una respuesta sin poder mostrarla». El invariante nombra a **quien administra la infraestructura**; el lado del mostrador lo cierra `RF-12`, que es el ítem que lo dice. Son dos fuentes y no una cláusula agregada | Con qué transformación se guarda la respuesta que `RF-68` le pide al emisor, dónde corre la comparación que `RF-69` habilita, cómo se cuentan los intentos de `RF-82` y qué caduca con el giro por `RF-83` — decisión **(f)** |
...
| Respuesta del desafío y sus intentos (`RF-68`, `RF-12`, `RF-82`, `RNF-17`) | Comprobar sin poder mostrar, **también frente al administrador de la infraestructura del giro** | **SQL en base propia, con credenciales propias, valor derivado con función lenta** | `RNF-17` prohíbe que la respuesta sea legible **para quien administre la infraestructura**, y su invariante dice que el sistema *puede comprobar una respuesta sin poder mostrarla*; el lado del mostrador lo prohíbe `RF-12`, que es un ítem aparte. Una columna de la base de giros incumple el primero por construcción, y una pantalla que la muestre incumple el segundo | Base separada, escrita solo por el servicio de desafío. Derivación lenta con sal por giro más una clave de servicio fuera de la base, de modo que ni con la base robada se prueban candidatos. Contador de intentos en la **misma** base, nunca en el giro (decisión **(f)**) |
...
| `POST` | `/giros/{id}/diferencias` | **Adaptador de canal → centro, por el receptor, desde su llamada o su mensaje de texto** (`RF-90`) | Registra la declaración de Elena de que el dinero recibido no coincide con lo informado, **después de identificarla como el receptor designado** (`RF-126`, ver `POST /receptores/verificacion`). **Ya no entra por el mostrador**: `RF-90` la hace entrar por el canal de Elena, y ese cambio importa porque el punto sobre el que se declara es aquel al que `RF-143` le descuenta la diferencia — pedirle a Kevin que la escriba era pedirle que se acusara solo. Toda diferencia registrada se informa **a las dos puntas**: al emisor (`RF-60`) y al receptor (`RF-121`); la comprobada se repone a quien le faltó (`RF-59`) y se descuenta al agente por el camino de `RF-104` y `RF-143`. Solo-adición. `IDEMP` | `RF-90`, `RF-126`, `RF-59`, `RF-60`, `RF-121` |
...
no tenían ninguna —la superficie del receptor (`RF-126`, `RF-128`), el vencimiento de la custodia
...
| Hacer llegar el código únicamente al emisor por su canal, **sin que quien atiende pueda leerlo, transcribirlo ni volver a consultarlo — y confirmándole que salió** | `RF-11`, `RF-94`, `RF-112`, `RF-113`, `RF-125`, `RNF-15` | Ventanilla de origen; la salida del código, adaptador de canal → teléfono registrado del emisor | `POST /giros` |
...
| Avisar al receptor por llamada o mensaje de texto: que hay un giro, cuánto cobrará, **la pregunta del desafío cuando no tiene documento vigente**, si el punto puede pagarle y a cuál ir si deja de poder, si quedó retenido, si se devolvió, qué puede hacer si una lista lo dejó sin cobrar, la diferencia registrada y la prescripción que se acerca | `RF-25`, `RF-41`, `RF-69`, `RF-75`, `RF-37`, `RF-106`, `RF-108`, `RF-109`, `RF-120`, `RF-121`, `RF-124` | Proceso interno hacia el canal de Elena | `POST /avisos` |
...
| **Identificar al receptor designado antes de admitirle nada fuera del mostrador** | `RF-126` | Adaptador de canal → centro, por el receptor | `POST /receptores/verificacion` |
| Registrar **por llamada o mensaje de texto** la declaración del receptor de que el dinero no coincide, y reponer a quien le faltó toda diferencia comprobada | `RF-90`, `RF-126`, `RF-59` | Adaptador de canal → centro, por el receptor | `POST /giros/{id}/diferencias` |
| **Consultar el estado de los giros a nombre del receptor, sin depender del emisor** | `RF-128`, `RF-126` | Receptor, sobre su sesión de canal | `GET /giros?receptor={id}` |
...
| Cancelar y devolver, incluido el giro cuyo código se perdió, y avisarle al receptor que se devolvió | `RF-46`, `RF-47`, `RF-110`, `RF-106`, `RNF-13` | Emisor, nunca el receptor (`RF-48`) | `POST /giros/{id}/cancelacion` |
| Rehacer al mismo receptor el giro cancelado, **conservando la cotización que fijó el cancelado** y **sin recobrar la comisión**, con los controles corridos otra vez enteros | `RF-117`, `RF-134`, `RF-135`, `RF-123`, `RF-110` | Emisor → ventanilla de origen | `POST /giros/{id}/rehacer` |
...
un permiso que alguien pueda concederse. La tercera es `GET /giros/{id}/motivo-retencion`: `RF-40`
...
2. **Con identidad comprobada antes de admitirle nada.** `RF-126` obliga a identificarla como **el
...
- **Con qué transformación entra.** Normalización decidida por el sistema —mayúsculas, acentos,
  ...
  declarable `RNF-17` frente a **quien administra la infraestructura**, que es el lector que el
  invariante nombra; el lado del mostrador no lo cierra este mecanismo sino `RF-12`, y por eso los
  dos se citan por separado y no como una cláusula sola.
...
- **Y sale por ahí y por ningún otro lado.** Este documento traía una segunda salida —imprimir el
  ...
  única salida sigue siendo cancelar y rehacer (`RF-110`, `RF-117`) — el mismo precio que ya se paga
