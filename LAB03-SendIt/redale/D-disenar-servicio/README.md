...
| `GET` | `/puntos/{id}/pendientes` | Punto de atención, por el operario | Lo que quedó a medias en ese mostrador, **en un solo estado que se lee y se retoma**: los giros interrumpidos (`RF-42`) y el efectivo recibido sin giro creado (`RF-118`), cada uno con la única acción que le corresponde. Solo lectura | `RF-42`, `RF-118`, `RF-39` |
...
ofrecerle a quien está delante** (`RF-39`), que en este caso es una sola y hay que decirla así:
volver cuando haya línea, o ir a otro punto si lo hay. `RF-39` no pide «una acción» en abstracto:
pide algo que Kevin pueda pronunciar en voz alta ante Elena, y «el sistema no está disponible» no lo
es.
