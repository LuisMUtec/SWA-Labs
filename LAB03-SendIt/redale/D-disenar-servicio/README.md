...
**Estado: decidido, y repropagado sobre los 236 ítems.** Las tres salidas del material están
elegidas —arquitectura, persistencia por familia de dato y diseño de la API— y las seis decisiones
que este caso obliga están cerradas, cada una con su alternativa descartada, el requisito que la
obliga y **lo que se pierde**. Ninguna tabla queda con `—`.

> **Deuda de repropagación: saldada.** La API estuvo ocho rondas contada contra el backlog de **155
> ítems**, con la deuda declarada en la propia línea del conteo. Se rehízo contra los **236**
> vigentes: **cuarenta y ocho rutas**, ocho de ellas nuevas. **Las seis decisiones no se movieron** —
> ningún título de las rondas 17 a 19 reabre el libro de partida doble, la idempotencia, la frontera
> con el punto, la cotización, el momento del tamizaje ni el camino del secreto—, y eso es
> exactamente lo que un corte arquitectónico bien puesto tiene que poder absorber. Lo que sí apareció
> es **un componente que no existía**: la identificación derivada al rol de cumplimiento (`RF-218`,
> `RF-219`, `RF-223`), que le da salida al receptor que agota los intentos del desafío. `A` lo modela
> y `L` lo dibuja.
...
| **Adaptadores de frontera** (uno por tercero) | Verificación del documento, listas de sanciones, proveedor de cotización, reporte a la autoridad, canal de voz y SMS | Cada uno falla, tarda o miente por su cuenta, y ninguno puede arrastrar el camino del pago. Tienen su propio reintento, su propia cola y su propio tiempo, medido aparte del techo de `RNF-09`. El canal de voz y SMS es además la **salida del código** de `RNF-15`: el único camino hacia el emisor que no pasa por el mostrador | `RF-01`, `RF-26`, `RF-119`, `RF-07`, `RF-17`, `RF-25`, `RF-98`, `RNF-15`, `RNF-09` |
...
**Convención de cierre legible, que se aplica a toda ruta llamada desde un mostrador:** la respuesta
incluye **en qué estado quedó la operación que el operario acaba de cerrar** (`RF-221`), con su
nombre y no con un código. No es una ruta más: es una columna más en la respuesta de las que ya hay,
y es lo que evita que Kevin tenga que adivinar si lo que hizo quedó hecho. `E` la cuenta como
`5 955` lecturas diarias, la mayor carga que un solo título agregó al paso.
...
| `POST` | `/identificaciones/{id}/segunda-opinion` | **Segundo rol, nunca el operario que atiende** | Resuelve la duda sobre si la fotografía del documento corresponde al emisor. **La decisión no es del criterio del operario** (`RF-225`): el mostrador abre la duda y otro rol la cierra, con su identidad y su momento. `IDEMP` | `RF-225`, `RNF-02` |
| `POST` | `/identificaciones/{id}/vencimiento` | Proceso interno, tarea minutada | Rechaza el giro cuya duda no se resolvió en `[ASSUMPTION: 10 minutos]`, **antes de que el operario reciba el efectivo** (`RF-229`) — de modo que no hay dinero que devolver ni asiento que revertir, que es la misma altitud en que `RF-93` rechaza | `RF-229`, `RF-79` |
...
| `POST` | `/giros/{id}/identificacion-derivada` | Punto de atención → centro, al agotarse los intentos | Abre la derivación al rol de cumplimiento del receptor que agotó los `[ASSUMPTION: 3]` intentos de `RF-82`. **Es la salida que el desafío no tenía**: antes, agotar los intentos dejaba a Elena sin camino. Deja el giro no pagable mientras dura, y arranca el reloj de `RF-219`. `IDEMP` | `RF-218`, `RF-82`, `RF-220` |
| `POST` | `/identificaciones-derivadas/{id}/resolucion` | **Rol de cumplimiento, en su tablero** | Identifica al receptor **contra los datos que el emisor registró al crear el giro** —no contra un documento que Elena no tiene— y habilita el pago. Exige motivo escrito y queda en la traza con su identidad. `IDEMP` | `RF-218`, `RNF-02`, `RF-232` |
| `POST` | `/identificaciones-derivadas/{id}/vencimiento` | Proceso interno, tarea diaria | **El terminador de la derivación.** Vence la que no se resolvió dentro de `[ASSUMPTION: 24 horas]` (`RF-219`) y devuelve el giro al emisor (`RF-223`), avisando a las dos puntas. Sin él, la derivación sería un estado sin quien lo cierre — la cuarta variante que `ITERACIONES.md` persigue por nombre | `RF-219`, `RF-223`, `RF-106` |
...
| `POST` | `/giros/{id}/diferencias` | **Adaptador de canal → centro, por el receptor, desde su llamada o su mensaje de texto** (`RF-90`) | Registra la declaración de Elena de que el dinero recibido no coincide con lo informado, **después de identificarla como el receptor designado** (`RF-196`, ver `POST /receptores/verificacion`). **Ya no entra por el mostrador**: `RF-90` la hace entrar por el canal de Elena, y ese cambio importa porque el punto sobre el que se declara es aquel al que `RF-143` le descuenta la diferencia — pedirle a Kevin que la escriba era pedirle que se acusara solo. Toda diferencia registrada se informa **a las dos puntas**: al emisor (`RF-60`) y al receptor (`RF-121`); la comprobada se repone a quien le faltó (`RF-59`) y se descuenta al agente por el camino de `RF-104` y `RF-143`. Solo-adición. `IDEMP` | `RF-90`, `RF-196`, `RF-59`, `RF-60`, `RF-121` |
...
| `POST` | `/avisos/{id}/redespacho` | **Adaptador de canal → centro, por el destinatario identificado** | Vuelve a despachar el aviso que su destinatario dice no haber recibido (`RF-227`), sobre la sesión de canal de `POST /receptores/verificacion` o la identidad del emisor. **Rechaza con `409` el aviso que lleva el código de seguimiento** (`RF-228`): es el «salvo» que sostiene a `RF-94`, y por eso vive acá y no en el proceso automático de `RF-202` | `RF-227`, `RF-228`, `RF-94` |
| `GET` | `/corredores/{origen}/{destino}/efectivo` | Proceso interno, antes de comprometer un giro | Efectivo disponible del corredor, **producido a partir de lo que declaran los puntos que lo integran** (`RF-235`) y no declarado aparte: es la cifra que `RF-20` comprueba al fondear, y hasta esta ronda ningún título la producía | `RF-235`, `RF-20`, `RF-76` |
...
| `POST` | `/reposiciones/{id}/retiro` | Punto elegido por el receptor → centro | Entrega en efectivo la reposición de una diferencia. Exige el **documento oficial vigente** del receptor designado (`RF-204`), o admite el **vencido cuando responde correctamente el desafío** (`RF-222`) — la misma llave que `RF-69` le dio para cobrar. Y **nunca repone más que lo que el giro fijó menos lo que la evidencia de entrega de `RF-168` acredita** (`RF-214`): el techo que la ronda 17 le puso a `RF-194`. `IDEMP` | `RF-194`, `RF-204`, `RF-222`, `RF-214`, `RF-168` |
...
| `POST` | `/supresiones` | Rosa → proceso interno, **identificada antes de admitirla**: contra los datos registrados si la pide fuera del mostrador (`RF-215`), contra documento oficial vigente si la pide en él (`RF-233`) | Suprime los datos personales **de quien los pide y de nadie más**, destruyendo la clave de su evidencia. No toca la traza de actos, que prevalece. `IDEMP` | `RF-81`, `RF-99`, `RNF-03` |

**Cuarenta y ocho rutas para cincuenta y dos capacidades**, contra el backlog vigente de **236
ítems** —216 funcionales y 20 no funcionales—. El conteo anterior —40 rutas sobre 155 ítems— quedó
sin valor cuando las rondas 17 a 19 agregaron veintiún títulos, y rehacerlo **no fue sumar ocho filas
a una tabla**: fue comprobar los veintiuno contra la superficie que ya existía. **Trece cayeron
dentro de una ruta que ya estaba** —`RF-226` y `RF-233` son compuertas de identidad sobre
`/giros/{id}/cancelacion` y `/supresiones`, `RF-230` es un rechazo de `/giros/{id}/pago`, `RF-234` una
cifra más en `GET /corredores`, `RF-221` una columna más en la respuesta de toda ruta de mostrador—,
y **ocho no tenían ninguna**:

| Ruta nueva | Por qué ninguna existente la servía |
|---|---|
| `POST /identificaciones/{id}/segunda-opinion` · `POST /identificaciones/{id}/vencimiento` | `RF-225` saca la duda sobre la fotografía del criterio del operario, y `RF-229` la vence **antes del efectivo**. No hay ruta de identificación que admita un segundo decisor |
| `POST /giros/{id}/identificacion-derivada` · `POST /identificaciones-derivadas/{id}/resolucion` · `POST /identificaciones-derivadas/{id}/vencimiento` | **Es el único componente que los veintiún títulos obligan a crear.** Agotar los intentos del desafío no tenía salida: `RF-218` se la da contra los datos que el emisor registró, `RF-219` le pone reloj y `RF-223` la termina devolviendo |
| `POST /reposiciones/{id}/retiro` | `RF-194` reponía desde la ronda 13 y **nadie entregaba la reposición**: no había ruta, y `RF-204`, `RF-222` y `RF-214` colgaban de un acto que la API no exponía |
| `POST /avisos/{id}/redespacho` | `RF-202` redespacha automático; `RF-227` lo hace **a pedido de quien se identifica**, y `RF-228` tiene que poder rechazar el del código. Son tres decisiones distintas sobre un mismo aviso |
| `GET /corredores/{origen}/{destino}/efectivo` | `RF-235` le da productor a la cifra que `RF-20` comprobaba desde la ronda 02 sin que nada la calculara |

**Y ninguna ruta vieja se cayó**, porque cada una se volvió a comprobar contra un título vivo: las
que colgaban de `RF-21`, `RF-45`, `RF-58`, `RF-107` o `RF-150` cuelgan ahora de lo que los cubre —el
último, de `RF-151` vía `RF-220`. Las capacidades
...
| Crear el giro con un receptor y un país únicos e inalterables, aplicando las reglas de los dos reguladores | `RF-03`, `RF-04`, `RF-16`, `RF-16`, `RF-23`, `RF-63`, `RF-70` | Ventanilla de origen | `POST /giros` |
| Hacer llegar el código únicamente al emisor por su canal, **sin que quien atiende pueda leerlo, transcribirlo ni volver a consultarlo — y confirmándole que salió** | `RF-11`, `RF-94`, `RNF-15`, `RF-125`, `RF-149`, `RNF-15` | Ventanilla de origen; la salida del código, adaptador de canal → teléfono registrado del emisor | `POST /giros` |
...
| **Resolver con un segundo rol la duda sobre la fotografía del documento, y rechazar antes del efectivo la que no se resuelve** | `RF-225`, `RF-229` | Segundo rol; proceso interno para el vencimiento | `POST /identificaciones/{id}/segunda-opinion`, `POST /identificaciones/{id}/vencimiento` |
| **Identificar al receptor que agotó los intentos del desafío, contra los datos que el emisor registró, y terminar la derivación** | `RF-218`, `RF-219`, `RF-223` | Rol de cumplimiento; proceso interno para el vencimiento | `POST /giros/{id}/identificacion-derivada`, `POST /identificaciones-derivadas/{id}/resolucion`, `POST /identificaciones-derivadas/{id}/vencimiento` |
| **Entregar la reposición de una diferencia, con documento vigente o con desafío, y sin pasar del techo que la evidencia acredita** | `RF-194`, `RF-204`, `RF-222`, `RF-214` | Punto elegido por el receptor | `POST /reposiciones/{id}/retiro` |
| **Redespachar un aviso a pedido de su destinatario identificado, salvo el que lleva el código** | `RF-227`, `RF-228` | Adaptador de canal, por el destinatario | `POST /avisos/{id}/redespacho` |
| **Producir el efectivo disponible de un corredor a partir de lo que declaran sus puntos** | `RF-235`, `RF-20` | Proceso interno | `GET /corredores/{origen}/{destino}/efectivo` |
| **Mostrar al operario en qué estado quedó la operación que acaba de cerrar** | `RF-221` | Todo mostrador | No es una ruta: es una columna en la respuesta de todas las de mostrador |
...
| **Identificar al receptor designado antes de admitirle nada fuera del mostrador** | `RF-196` | Adaptador de canal → centro, por el receptor | `POST /receptores/verificacion` |
| Registrar **por llamada o mensaje de texto** la declaración del receptor de que el dinero no coincide, y reponer a quien le faltó toda diferencia comprobada | `RF-90`, `RF-196`, `RF-59` | Adaptador de canal → centro, por el receptor | `POST /giros/{id}/diferencias` |
| **Consultar el estado de los giros a nombre del receptor, sin depender del emisor** | `RF-128`, `RF-196` | Receptor, sobre su sesión de canal | `GET /giros?receptor={id}` |
...
decía lo que `RNF-15` y `RNF-15` ya dicen, y se retiró. La tercera es `GET /giros/{id}/motivo-retencion`: `RF-40`
...
2. **Con identidad comprobada antes de admitirle nada.** `RF-196` obliga a identificarla como **el
...
**Y la salida, que es el código (`RF-11`, `RNF-15`, `RF-125`).** El código lo genera el sistema y
...
`RNF-15` cierra esa puerta —*impedirá que quien atiende el mostrador lea o transcriba el código*— y
...
- **El dispositivo orientado al cliente es hardware en un local que no es de SendIt**, y su clave
  ...
  que contarlas: `RNF-15` y `RF-114` no se cumplen con software solo.
