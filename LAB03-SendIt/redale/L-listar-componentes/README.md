...
**Estado: tres iteraciones dibujadas y trazabilidad completa.** Las 155 filas del backlog —138 `RF` y
17 `RNF`— tienen componente e iteración. Lo que queda abierto está marcado como abierto y no como
hecho.

**Repropagación de la ronda 11.** El backlog pasó de 137 a 155 ítems y retiró cuatro identificadores.
...
`RF-108` avisa a quien deja de poder cobrar. **No se rehízo la pasada: se corrigió una caja que
...
| `Screening Job <actualización de lista>` | Re-tamiza **los giros aún no pagados** cuando cambia una lista de sanciones (`RF-97`), que es el caso que `D`(e) señaló como imposible de resolver «antes o después». Baja acá desde la iteración 2 porque allá cargaba `RF-27`, que es otra cosa; y acá tiene además el canal que le faltaba: avisa por llamada o mensaje de texto a quien deja de poder cobrar por el cambio de lista que su giro será devuelto al emisor (`RF-108`) | `D`(e) — el momento del tamizaje | `RF-97`, `RF-108` |
...
| `Reclamo Service` | El caso de diferencia declarada por el receptor: abre lo que el `Corrección Service` resuelve. Le llega por dos superficies —el mostrador, y el teléfono a través del `Atención al Receptor Service`— y **ninguna de las dos lo admite sin identificación previa**. Sin esta caja, la diferencia solo se detecta en el cierre de caja y del lado de SendIt | `R` — `BR-13` | `RF-90` junto al `Atención al Receptor Service` |
| `Atención al Receptor Service` | **La entrada que faltaba, y el hallazgo más caro que este paso arrastraba.** El `Operador de voz y mensaje de texto` tenía dieciséis avisos salientes y **cero entradas**, de modo que `RF-90` —declarar por llamada o mensaje de texto que el dinero recibido no coincide— no tenía por dónde entrar, mientras el descuento que dispara sí estaba dibujado. Esta caja recibe lo que el receptor origina por ese canal y **no admite nada sin control de identidad**: le pregunta al `Identity Service` si quien llama es **el receptor designado** *antes* de admitir la declaración (`RF-126`), y recién entonces la pasa al `Reclamo Service` (`RF-90`) o le devuelve el estado de los giros a su nombre (`RF-128`). Todo lo que entra por acá deja traza en el `Audit Service`, porque es la única superficie del diseño donde una persona que no está frente a un mostrador mueve dinero ajeno | `R` — `BR-13`, `BR-09`, `BR-01` · `A` — la identificación del receptor es una verificación con evidencia, no un nombre dicho por teléfono | `RF-90`, `RF-126`, `RF-128` |
...
| `Identity Service` (ampliado) | El mismo de la iteración 2, con dos usos que la 2 no tenía. **Registra la fecha de nacimiento del emisor y del receptor cuando el reporte a una autoridad la exige** (`RF-140`) — un dato que existe por el regulador y no por la operación, y por eso entra acá y sale solo hacia el `Reporte Service`. Y es quien contesta si **quien llama es el receptor designado** antes de que el `Atención al Receptor Service` admita nada (`RF-126`) | `A` — entidad *Verificación de identidad* | `RF-140`; y `RF-126` junto al `Atención al Receptor Service` |
...
**155 filas: 138 `RF` y 17 `RNF`.** Una por ítem vivo del backlog, ninguna agrupada, ninguna vacía,
...
El reparto: **1** fila en la iteración 1, **36** en la 2 (29 `RF` + 7 `RNF`) y **118** en la 3
(108 `RF` + 10 `RNF`). Los títulos de abajo se **extraen del backlog**, no se transcriben a mano: es
...
| RF-113 | el sistema impedirá que quien atiende el mostrador lea o transcriba el código de seguimiento - Kevin | **3** — Código Service → Terminal del Cliente | `DONE` |
...
| RF-110 | el sistema permitirá al emisor cancelar un giro cuyo código se perdió, mientras el dinero no esté a disposición del receptor - Rosa | **3** — Cancelación Service | `DONE` |
| RF-117 | el sistema permitirá rehacer al mismo receptor el giro cancelado por código perdido - Rosa | **3** — Rehacer Service | `DONE` |
...
| RF-140 | el sistema registrará la fecha de nacimiento del emisor y del receptor cuando el reporte a una autoridad la exige - Kevin | **3** — Identity Service → Reporte Service | `DONE` |
...
| RF-108 | el sistema informará por llamada o mensaje de texto a quien deja de poder cobrar por un cambio de lista que su giro será devuelto al emisor - Elena | **3** — Screening Job &lt;actualización de lista&gt; → Notification Service | `DONE` |
...
| RF-137 | el sistema dará por prescrita la devolución que nadie retiró al vencer el plazo de prescripción del giro - Rosa | **3** — Vencimiento Job &lt;diario&gt; → Devolución Service | `DONE` |
...
| RF-93 | el sistema retendrá el giro cuyo receptor superó su acumulado de cobros en la ventana `[ASSUMPTION: 30 días corridos]` - Kevin | **3** — Cobros del Receptor Service → Retención Service | `DONE` |
...
| RF-86 | el sistema exigirá que el documento del receptor esté vigente, salvo cuando responde correctamente el desafío - Elena | **3** — Identity Service ↔ Desafío Service | `DONE` |
...
| RF-104 | el sistema no aplicará a la liquidación del agente ningún descuento por diferencia que no le haya informado antes con la evidencia que lo sustenta - Elena | **3** — Descuento al Agente Service | `DONE` |
| RF-143 | el sistema descontará de la liquidación del agente la diferencia repuesta que se originó en su punto - Elena | **3** — Descuento al Agente Service → Liquidación Job &lt;plazo del agente&gt; | `DONE` |
...
| RF-126 | el sistema identificará como el receptor designado a quien declara una diferencia fuera del mostrador, antes de admitir su declaración - Elena | **3** — Atención al Receptor Service → Identity Service | `DONE` |
...
| RF-129 | el sistema devolverá al emisor el giro para el que no queda ningún punto de atención capaz de pagarlo en el distrito elegido - Rosa | **3** — Punto Alternativo Service → Devolución Service | `DONE` |
...
| RF-111 | el sistema devolverá al emisor el giro cuyo punto no puede pagar y cuya respuesta no llega dentro del plazo que se le informó al ofrecerle otro - Rosa | **3** — Punto Alternativo Service → Devolución Service | `DONE` |
| RF-116 | el sistema informará al emisor, al ofrecerle otro punto de atención, dentro de cuánto debe responder, y ese plazo no será menor que `[ASSUMPTION: 24 horas]` - Rosa | **3** — Punto Alternativo Service | `DONE` |
| RF-39 | el sistema asociará al menos una acción ofrecible a todo giro rechazado, retenido o no pagable - Kevin | **2** — Giro Service | `DONE` |
...
| RF-141 | el sistema informará al agente la diferencia entre el efectivo esperado y el declarado en cada cierre de caja de sus puntos - Kevin | **3** — Cierre de Caja Job &lt;EOD&gt; → Descuento al Agente Service | `DONE` |
...
| RF-47 | el sistema devolverá al emisor, al cancelar, el efectivo que entregó, comisión incluida - Rosa | **3** — Devolución Service | `DONE` |
...
| RF-128 | el sistema permitirá al receptor consultar el estado de los giros a su nombre sin depender del emisor - Elena | **3** — Atención al Receptor Service → Consulta Service | `DONE` |
...
| RF-131 | el sistema registrará si el aviso llegó a su destinatario, cuando el canal lo reporta - Elena | **3** — Notification Service → Audit Service | `DONE` |
...
| RNF-02 | el sistema impedirá que una misma identidad ejerza dos roles sobre un giro, y que quien atiende un mostrador inicie una corrección sobre un giro pagado | **3** — Dual Control Service | `DONE` |
...
**Cuatro identificadores retirados, y sus números quedan muertos.** No se reutilizan y no quedan
...
1. **¿Qué entra desde afuera y se cree sin verificar?** La tasa del proveedor y el resultado de las
   ...
   verifica antes de creerse: `RF-126` exige identificarlo como el receptor designado **antes** de
   admitir la declaración, porque aguas abajo esa palabra mueve dinero.
...
- [x] El canal de voz tiene entrada además de salida, y esa entrada tiene control de identidad (RF-126)
...
- [x] Ningún ítem del backlog sin componente que lo cubra — 155 filas, una por ítem vivo, sin huecos y sin punteros a identificadores retirados
