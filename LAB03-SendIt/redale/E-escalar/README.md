...
**Los tres hallazgos, antes de las tablas.** (1) El sistema **nace pasado su primer cuello**: la
cuota del operador de telefonía se agota en `1 360` giros diarios y `E` estima la llegada en `2 800`.
(2) El plano único de escritura que [`D`](../D-disenar-servicio/README.md)(a) eligió deja de caber en
una máquina en **`34 700` giros/día** — entre el segundo tramo y el tercero, y mucho antes de que
...
| **Consume de D** | Las **seis** decisiones, ya cerradas. Tres tienen tramo de vencimiento y aquí se nombra cuál: **(a)** el libro de partida doble solo-adición en un plano único de escritura, que deja de caber en una máquina en `34 700` giros/día; **(c)** el arrendamiento exclusivo solo en línea, que **no** vence —elimina la divergencia por construcción— pero convierte cada caída de enlace en mostrador cerrado, y esa cuenta crece con los puntos; **(f)** el servicio de desafío, que `D` puso con base y credenciales propias y al que la tabla de once servidores de `E` **no le dio nodo** |
...
ninguno: `2,11` ventanillas por giro, `3,0` de pico diario (el componente **diario** de `P`, no el
compuesto de `11`), `30,4 / 2` giros por emisor y mes (`S-04`), `4,9` avisos por giro (`S-14`) y
`0,96` pagos por giro (`S-13`).
...
|---|---|---|---|---|---|---|
| `< 1 k` | **2 110** — menos que las `9 000` declaraciones de caja del mismo día | **3 000** | **15 200** | **3 000** — piso de `S-12`, no derivado del volumen | **4 900** | **2** |
| `1 k – 10 k` | **21 100** — SendIt entra acá, en `2 800` (`5 921` ventanillas) | **30 000** | **152 000** | **3 000** — la red sigue al `11 %` de su techo de caja | **49 000** | **2** |
| `100 k` | **211 000** | **300 000** | **1 520 000** | **3 200** — primer tramo en que la red deja de estar sobrada | **490 000** | **2**, al `18 %` de los dos corredores enteros |
| `500 k` | **1 055 000** | **1 500 000** | **7 600 000** | **16 000** | **2 450 000** | **≥ 12** — `89 %` de los dos corredores es inalcanzable |
| `1 M` | **2 110 000** | **3 000 000** | **15 200 000** | **32 000** | **4 900 000** | **≥ 24** — `1,8×` los dos corredores enteros |
...
`USD 245/día` en el tramo bajo, `USD 245 000/día` en el alto, y una cuota que se agota mucho antes
que el presupuesto.
...
de magnitud —`E` calculó `0,09` para once veces esta carga— y **ya se rompen dos cosas**, ninguna de
máquina.

| Qué se rompe primero | Por qué | Qué cambia en la arquitectura |
|---|---|---|
| **La cuota diaria del operador de telefonía**, en `1 360` giros/día | `4,9` avisos por giro (`S-14`) por `3,0` de pico diario son `14,7` unidades por giro el día señalado. Contra la cuota de `10 000/día` por país sin acuerdo comercial que `E` supuso, los dos países de `S-01` dan `20 000`: `20 000 ÷ 14,7 = 1 360`. **El tramo se rompe adentro, y por debajo del volumen de llegada de `2 800`** — SendIt nace ya pasado su primer cuello. Los cuatro avisos P0 (RF-25, RF-37, RF-41, RF-75) y el P1 (RF-60) salen por ahí y no tienen plan B: Elena no tiene aplicación ni datos | **Nada en el diagrama; todo en el contrato.** El `Notification Service` que [`L`](../L-listar-componentes/README.md) dibuja como una caja pasa a ser **uno por país** —RNF-12 obliga a contar dos operadores, no dos endpoints— y gana lo único que la arquitectura puede aportar: cola con prioridad, para que la cuota se agote en RF-60 (P1) y no en RF-25 (P0), y constancia de RF-98 también de lo **no** entregado, que hoy solo registra lo enviado |
| **El mostrador sin línea — que ya no diverge: cierra** | `D`(c) cerró la divergencia y dejó el precio escrito: sin arrendamiento no hay pago, y el arrendamiento solo se obtiene en línea (RF-100, RNF-06). El riesgo no crece con los giros sino con los `3 000` puntos que `S-12` pone desde el día uno, y no se manifiesta como dinero pagado dos veces sino como Elena volviendo en combi sin nada, después de que RF-41 y RF-75 le prometieran que ese punto podía pagarle. La salida de RF-91 tampoco corre: rederivar exige línea | **Este es el tramo donde se paga la deuda que `D`(c) le dejó a este paso por escrito**: dimensionar un segundo camino de conectividad por punto, o aceptar por escrito la indisponibilidad que RNF-11 va a comerse. El `Sync Service` no es la respuesta —cuando el punto vuelve no hay nada que reconciliar, no pagó nada—; la respuesta está aguas abajo del diagrama, en el enlace |
| **El piso de RF-76 es mayor que el negocio entero** | `9 000` declaraciones de caja al día contra `2 110` operaciones de ventanilla: la carga que no se apaga es `4,3×` la que sí. Manda hasta `9 000 ÷ 2,11 = ` **4 265 giros/día**, es decir todo este tramo y la mitad del siguiente, incluida la llegada de `2 800` | No es una rotura, es el **suelo del tramo**: el `Caja del Punto Service` y `BD puntos de atención y efectivo` se dimensionan contra `S-12`, nunca contra el volumen. Un día sin un solo giro sigue escribiendo `9 000` filas |
| **Nada de máquina** | La carga pide `0,03` servidores y RNF-11 pide `11`. El sistema arranca sobreprovisionado `×370` y sigue estándolo todo el tramo | Ninguno. Subir dentro de este tramo no cambia una sola caja del diagrama |
...
| Qué se rompe primero | Por qué | Qué cambia en la arquitectura |
|---|---|---|
| **La cuota de telefonía, ya rota, se vuelve estructural** | En el techo del tramo son `49 000` avisos/día y `147 000` el día señalado: `7,4×` los `20 000` de cuota. Un factor de dos se negocia en un mes; uno de siete cambia el modelo comercial del canal, y con él el `USD 0,24` por giro que `S-14` cargó al costo unitario | Deja de ser un adaptador y pasa a ser una **decisión de compra por corredor**: cada país nuevo trae operador, cuota, precio y tasa de entrega propios. Es el único cuello del sistema que se renegocia en vez de rediseñarse |
| **El caudal de la cola de cumplimiento, con dos términos que no se parecen** | `E` estimó `70` casos/día a `2 800` giros: `56` por tamizaje (`S-08`, `2 %` sobre la **identidad**) y `14` por el acumulado de cobros de RF-93 (`0,5 %`, sobre la **frecuencia**). En el techo del tramo el primero da `200/día` —crece igual que el volumen, porque la tasa de sanciones no depende de cuánto gira la gente— y el segundo se dispara: cuando SendIt tiene éxito en un corredor sube `cobros por receptor`, que es justo lo que RF-58 cuenta y RF-93 castiga. Con `2,6` cobros por receptor en vez de los `2,2` de `S-15` `[ASSUMPTION]`, RF-93 aporta `80/día` y el total es `280`, no `250` | El `Retención Service` deja de tener **una** cola y pasa a tener dos entradas medidas aparte, porque no se dimensionan con la misma fórmula: una es `giros × tasa fija`, la otra es `giros × f(giros por receptor)`. Dimensionar la planta de analistas contra la tasa de sanciones subestima el segundo término exactamente en los tramos donde el negocio va bien |
| **El desafío deja de ser un caso raro** | `S-16` lo puso en el `20 %` de los pagos a `1,6` intentos: `3 200` comprobaciones/día en el techo del tramo. Cada una atraviesa el `Desafío Service`, que `D`(f) construyó con **derivación lenta a propósito** y comparación **solo en el centro**. RF-82 la invoca en cada intento y RF-103 devuelve el giro al tercero | Todavía no se rompe, pero aquí se ve la deuda: `E` repartió sus once servidores en cuatro planos —escritura, réplica de lectura, consulta y cumplimiento— y **ninguno es el del desafío**, que `D`(f) exige con proceso, base y credenciales propios. RNF-11 mide sobre *pagar*, y el `20 %` de los pagos pasa por un componente que no tiene nodo asignado |
| **Nada de máquina, otra vez** | En el techo del tramo la escritura pide `0,29` servidores en pico contra los `80 req/s` de un plano | Ninguno. Los once servidores siguen siendo de RNF-11 y de la geografía, no de la carga |
...
| Qué se rompe primero | Por qué | Qué cambia en la arquitectura |
|---|---|---|
| **El plano único de escritura de `D`(a)**, ya roto en `34 700` giros/día | `E` desglosó `50 729` escrituras diarias sobre `2 800` giros: `18,12` por giro, porque `D`(a) hizo que **todo hecho sea un asiento con contrapartida** —la retención de RF-29 no mueve un centavo y reclasifica `Obligación con el receptor` contra `Obligación retenida`— y porque RF-07 hizo de la cotización una escritura durable antes del pago. En pico: `giros × 18,12 ÷ 86 400 × 11 = 80 req/s` en **`34 700` giros/día**, que es la capacidad de un plano de escritura de 16 vCPU. A `100 k` hacen falta `2,9`. Los tres nodos de RNF-11 son **un** plano con quórum, no tres capacidades | Aparece el **particionamiento del libro**, y con él la pregunta que este documento venía dejando abierta: por qué clave. La respuesta la fija `D`, no este paso: **por giro**, porque RNF-07 exige que las dos líneas del asiento entren juntas y las dos son del mismo giro. Lo que queda a caballo es lo que `E` ya había señalado con `1,3` requests al día: RF-81 busca a una persona **a través de** todos sus giros, y RF-58/RF-93 cuentan por **identidad de receptor**, que `D` obliga a agregar dentro de la transacción de autorización y sin caché |
...
| **El módulo del desafío, ahora sí, como dependencia dura del pago** | `32 000` comprobaciones/día (RF-82) más `100 000` escrituras de RF-68 —que no admite excepción— pasan por una **derivación lenta por diseño**. En pico son `16,8/s` de derivación; a `[ASSUMPTION: 200 ms]` por derivación son `3,4` núcleos dedicados a un cómputo que se eligió caro a propósito. El costo no es el problema: el problema es que `D`(f) puso la comparación **solo en el centro** y RNF-11 mide sobre *pagar*. Si `BD desafíos` no contesta, el `20 %` de los pagos no se completa y RF-103 empieza a devolver giros de receptores legítimos | El `Desafío Service` deja de ser un proceso aislado por confidencialidad y pasa a ser **un plano con su propio quórum**, sin dejar de tener credenciales separadas —que es el motivo por el que existe (RNF-17)—. Esto **invalida el reparto de once servidores de `E`**, que no le asignó ninguno: no se recalcula acá (no es lo que este paso hace), se devuelve a [`E`](../E-estimar/README.md) como fila nueva de su tabla de planos |
| **La capacidad de decidir dentro del plazo de la retención** | `2 000` casos de sanciones más el término de RF-93, que a esta profundidad de red ya no es el `0,5 %` de `S-08`: son del orden de `3 500` casos/día que hay que resolver **dentro del plazo de RF-31**. Lo que satura no se apila —RF-32 vence y devuelve— así que la señal es **tasa de devolución automática**, no retraso | Ninguna caja nueva. La única palanca que la arquitectura tiene es achicar el caudal, y ya está construida: RF-78 separa homonimia de coincidencia exacta y el `98 %` son homónimos (`S-08`). Convertir esa mayoría en una decisión del sistema no es un cambio de `L`: es un ítem de `R` que alguien tiene que aceptar |
...
| 5 | La capacidad de decidir dentro del plazo de la retención | `100 k`, y creciendo más rápido que el volumen por RF-93 | Personas, o achicar el caudal con RF-78 — que es un ítem de `R` |
...
que ocurre **antes** del pago. De ahí salen las `18,12` escrituras por giro que `E` desglosó, y de
ahí sale el tramo: `18,12 × giros ÷ 86 400 × 11 = 80 req/s` en **`34 700` giros/día**. Ese es el
número, y llega mucho antes que los `341 000` en que la carga alcanzaría al piso de RNF-11.
...
es exactamente lo que el tramo `500 k` rompe.
...
| Red afiliada, operador de telefonía, listas y forma de identidad válida | Constante por corredor: un contrato de afiliación (RF-24), un operador (RF-25), una fuente de listas (RF-26, RF-27) y un catálogo de documento (RF-01) | **No.** Escala mal por precio, no por combinatoria |
...
| **La decisión `D`(a)** — el asiento con contrapartida para todo hecho del giro | El tramo de `34 700` giros/día en que el plano único de escritura deja de caber, que es función directa de las `18,12` escrituras por giro. Menos asientos por giro corre el tramo hacia arriba; más, hacia abajo |
| El reparto de los trece servidores de `E` | Nada de este paso, salvo una cosa: hoy **no hay nodo para el `Desafío Service`**, que `D`(f) exige con proceso y base propios y por el que pasa el `20 %` de los pagos que RNF-11 mide. Se devuelve a [`E`](../E-estimar/README.md) como fila nueva de su tabla de planos, no se recalcula acá |
