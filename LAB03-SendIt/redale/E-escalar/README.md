...
lo nace por más**: la cuota del operador de telefonía se agota en `1 333` giros diarios y `E` estima
la llegada en `2 800`, `2,1×` por encima. Los cinco avisos que el backlog agregó —RF-106, RF-107,
RF-108, RF-120 y RF-124— subieron el factor de `S-14` de `4,9` a `5,0` unidades por giro, y el cuello
bajó de `1 360` a `1 333`. (2) El plano único de escritura que
[`D`](../D-disenar-servicio/README.md)(a) eligió deja de caber en una máquina en **`29 100`
giros/día** —era `34 700`, y lo bajaron las escrituras nuevas de RF-119, RF-113 y RF-115—, todavía
entre el segundo tramo y el tercero, y mucho antes de que `RNF-11` deje de mandar. (3) Los dos tramos
altos del material **no son tramos de carga sino de mercado**: `1 M` giros/día a ticket de `USD 300`
son `USD 109 500 M/año`, **1,8 veces** los dos corredores enteros que `S-01` fijó.
...
| **Consume de D** | Las **seis** decisiones, ya cerradas. Tres tienen tramo de vencimiento y aquí se nombra cuál: **(a)** el libro de partida doble solo-adición en un plano único de escritura, que deja de caber en una máquina en `29 100` giros/día; **(c)** el arrendamiento exclusivo solo en línea, que **no** vence —elimina la divergencia por construcción— pero convierte cada caída de enlace en mostrador cerrado, y esa cuenta crece con los puntos; **(f)** el servicio de desafío, que `D` puso con base y credenciales propias y al que `E` **sí le da nodo**: es el **quinto plano** de su tabla de trece servidores, con `2`, y RF-113 y RF-114 lo ensancharon al sacar de la pantalla del mostrador tanto el código como la respuesta. Lo que sigue abierto no es si tiene nodo, sino si `2` alcanzan cuando el `20 %` de los pagos pasa por ahí |
...
porque RF-90 le da al receptor un acto propio en el mostrador, porque RF-91 parte un solo cobro en
...
ninguno: `2,13` ventanillas por giro (`5 965 / 2 800`, con el rehacer de RF-117 adentro), `3,0` de
pico diario (el componente **diario** de `P`, no el compuesto de `11`), `30,4 / 2` giros por emisor y
mes (`S-04`), `5,0` avisos por giro (`S-14`, ya con RF-106, RF-107, RF-108, RF-120, RF-121 y RF-124)
y `0,96` pagos por giro (`S-13`).
...
|---|---|---|---|---|---|---|
| `< 1 k` | **2 130** — menos que las `9 000` declaraciones de caja del mismo día | **3 000** | **15 200** | **3 000** — piso de `S-12`, no derivado del volumen | **5 000** — ya por encima de un cuarto de la cuota | **2** |
| `1 k – 10 k` | **21 300** — SendIt entra acá, en `2 800` (`5 965` ventanillas) | **30 000** | **152 000** | **3 000** — la red sigue al `11 %` de su techo de caja | **50 000** | **2** |
| `100 k` | **213 000** | **300 000** | **1 520 000** | **3 200** — primer tramo en que la red deja de estar sobrada | **500 000** | **2**, al `18 %` de los dos corredores enteros |
| `500 k` | **1 065 000** | **1 500 000** | **7 600 000** | **16 000** | **2 500 000** | **≥ 12** — `89 %` de los dos corredores es inalcanzable |
| `1 M` | **2 130 000** | **3 000 000** | **15 200 000** | **32 000** | **5 000 000** | **≥ 24** — `1,8×` los dos corredores enteros |
...
`5 000 × USD 0,05 = USD 250/día` en el tramo bajo, `USD 250 000/día` en el alto, y una cuota que se
agota mucho antes que el presupuesto. **Los cinco avisos nuevos la movieron `+2 %` entera**, y ese
`2 %` es lo que separa `1 360` de `1 333` giros/día en la fila de abajo.
...
|---|---|---|
| **La cuota diaria del operador de telefonía**, en `1 333` giros/día | `5,0` avisos por giro (`S-14`) por `3,0` de pico diario son `15,0` unidades por giro el día señalado. Contra la cuota de `10 000/día` por país sin acuerdo comercial que `E` supuso, los dos países de `S-01` dan `20 000`: `20 000 ÷ 15,0 = ` **1 333**, contra los `1 360` de antes de que el backlog agregara RF-106, RF-107, RF-108, RF-120 y RF-124. **El tramo se rompe adentro, y por debajo del volumen de llegada de `2 800`** —`2 800 / 1 333 = 2,1×` pasado— : SendIt nace ya pasado su primer cuello, y ahora lo nace por más. Del lado del receptor salen por ahí **nueve avisos P0** (RF-25, RF-37, RF-41, RF-69, RF-75, RF-106, RF-107, RF-109, RF-124) y **tres P1** (RF-108, RF-120, RF-121); del lado del emisor, cinco (RF-14, RF-51, RF-60, RF-65, RF-67). Ninguno tiene plan B: Elena no tiene aplicación ni datos | **Nada en el diagrama; todo en el contrato.** El `Notification Service` que [`L`](../L-listar-componentes/README.md) dibuja como una caja pasa a ser **uno por país** —RNF-12 obliga a contar dos operadores, no dos endpoints— y gana lo único que la arquitectura puede aportar: cola con prioridad, para que la cuota se agote en RF-121 y RF-120 (P1) y no en RF-25 ni en RF-107 (P0), y constancia de RF-98 también de lo **no** entregado, que hoy solo registra lo enviado. **La prioridad ahora tiene con qué ordenar**: antes había un solo P1 que sacrificar, ahora hay tres |
...
| **Nada de máquina** | La carga pide `0,10 × 1 000 / 2 800 = ` **`0,04`** servidores y RNF-11 con la geografía piden `13`. El sistema arranca sobreprovisionado `13 ÷ 0,036 = ` **`×360`** y sigue estándolo todo el tramo. Las escrituras que RF-119, RF-113 y RF-115 agregaron subieron la carga un `19 %` y no movieron una caja | Ninguno. Subir dentro de este tramo no cambia una sola caja del diagrama |
...
| Qué se rompe primero | Por qué | Qué cambia en la arquitectura |
|---|---|---|
| **El plano único de escritura de `D`(a)**, ya roto en `29 100` giros/día | `E` desglosó `60 527` escrituras diarias sobre `2 800` giros: `21,62` por giro —eran `18,12`— porque `D`(a) hizo que **todo hecho sea un asiento con contrapartida** —la retención de RF-29 no mueve un centavo y reclasifica `Obligación con el receptor` contra `Obligación retenida`—, porque RF-07 hizo de la cotización una escritura durable antes del pago, y porque el backlog agregó `3,50` escrituras por giro que antes no existían: el tamizaje del receptor antes del efectivo (RF-119) y su repetición sobre el giro rehecho (RF-123), el traspaso del código y la captura de la respuesta (RF-113, RF-114) y la constancia de los plazos informados (RF-115, RF-116). En pico: `giros × 21,62 ÷ 86 400 × 11 = 80 req/s` en **`29 100` giros/día**, que es la capacidad de un plano de escritura de 16 vCPU. A `100 k` hacen falta `3,4`, no `2,9`. Los tres nodos de RNF-11 son **un** plano con quórum, no tres capacidades | Aparece el **particionamiento del libro**, y con él la pregunta que este documento venía dejando abierta: por qué clave. La respuesta la fija `D`, no este paso: **por giro**, porque RNF-07 exige que las dos líneas del asiento entren juntas y las dos son del mismo giro. Lo que queda a caballo es lo que `E` ya había señalado con `1,3` requests al día: RF-81 busca a una persona **a través de** todos sus giros, y RF-58/RF-93 cuentan por **identidad de receptor**, que `D` obliga a agregar dentro de la transacción de autorización y sin caché |
...
**tres**: RF-93 retiene también al giro cuyo receptor superó el acumulado de cobros de la ventana que
RF-58 lleva, y RF-123 vuelve a meter en la cola cada giro rehecho tras perderse el código, que
...
|---|---|---|---|
| 1 | La cuota del operador de telefonía | `1 333` giros/día — **`2,1×` debajo del volumen de llegada** | Un contrato, no una máquina |
...
| 3 | El plano único de escritura de `D`(a) | `29 100` giros/día | Particionar el libro por giro — y asumir lo que queda a caballo |
| 4 | La caja por punto de atención | `93 750` giros/día | Tesorería del agente y RF-91 como balanceo, no como excepción |
| 5 | La capacidad de decidir dentro del plazo de la retención | `100 k`, y creciendo más rápido que el volumen por RF-93, con el reproceso de RF-123 encima | Personas, o achicar el caudal con RF-78 — que es un ítem de `R` |
...
adelantó de `34 700` a `29 100` giros/día sin dejar de ser el tercero.
...
que ocurre **antes** del pago. De ahí salen las `21,62` escrituras por giro que `E` desglosó —eran
`18,12` antes de que RF-119, RF-113, RF-114, RF-115, RF-116 y RF-123 agregaran `3,50` entre las
seis—, y de ahí sale el tramo: `21,62 × giros ÷ 86 400 × 11 = 80 req/s` en **`29 100` giros/día**.
Ese es el número, y llega mucho antes que los `364 000` en que la carga alcanzaría al piso de RNF-11.
**Cada control nuevo que el backlog acepta corre este tramo hacia abajo**, y lo corre en proporción
directa: `3,50` escrituras más por giro adelantaron el quiebre `5 600` giros/día.

La clave la fija RNF-07 y no hay elección: **se parte por giro**, porque las dos líneas del asiento
tienen que entrar juntas y las dos son del mismo giro. El doble pago no depende de la partición
—`D`(b) lo ancló en el índice único `(giro, acto)` del libro, que es el único mecanismo que ve dos
puntos desconectados a la vez—. Lo que sí queda a caballo son las tres operaciones que **no** se
indexan por giro: RF-81, que busca a una persona a través de todos sus giros (`1,3` requests al día,
y aun así decisivos); RF-58 y RF-93, que cuentan por identidad de receptor y que `D` obligó a agregar
dentro de la transacción de autorización y sin caché; y RF-08 y RF-09, el gemelo del lado emisor. Las
...
no queda a caballo, sino en otra partición del todo:** RF-113 y RF-114 mueven el código y la respuesta
al módulo del desafío, que `D`(f) tiene con base y credenciales propias — de modo que lo que se parte
por giro es el libro, no el secreto, y RF-112 le agrega la única negación que ese módulo ejerce.
...
`o × d` corredores y **`o × d` pares de reguladores** que RF-15 aplica de a dos sobre el mismo giro y
...
| Red afiliada, operador de telefonía, listas y forma de identidad válida | Constante por corredor: un contrato de afiliación (RF-24), un operador (RF-25), una fuente de listas (RF-26, RF-119, RF-27) y un catálogo de documento (RF-01, RF-86, RF-122) | **No en cantidad; sí en precio unitario**: el operador nuevo se contrata contra `5,0` unidades por giro, no contra `4,9`, y ese factor lo fija el backlog, no el corredor |
...
| Las fuentes de retención (RF-29 por sanciones, RF-93 por acumulado de cobros, RF-123 por reproceso del giro rehecho) | El caudal de la cola, que tiene un término que no escala con el volumen, otro que escala más rápido y un tercero que escala con el reproceso y no con el negocio. Una fuente nueva se suma aquí, no en la tabla de tramos |
...
| **La decisión `D`(a)** — el asiento con contrapartida para todo hecho del giro | El tramo de `29 100` giros/día en que el plano único de escritura deja de caber, que es función directa de las `21,62` escrituras por giro. Menos asientos por giro corre el tramo hacia arriba; más, hacia abajo — y las `3,50` que el backlog agregó ya lo corrieron `5 600` giros/día hacia abajo |
| El reparto de los trece servidores de `E` | Nada de este paso, salvo una cosa, y **ya no es la que esta fila decía**: el `Desafío Service` **sí tiene nodo** —es el quinto plano de `E`, con `2`— y lo que se le devuelve a [`E`](../E-estimar/README.md) es un cambio de tamaño, no un plano que falta. Por esos `2` nodos pasa el `20 %` de los pagos que RNF-11 mide, más el traspaso del código de RF-113 (uno por giro) y la captura de la respuesta de RF-114. No se recalcula acá |
| La cuota, el precio o la cobertura del operador de telefonía (`S-14`) | El cuello del canal de voz y SMS —`20 000 ÷ (5,0 × 3,0) = 1 333` giros/día— y con él la viabilidad de los **doce** avisos al receptor y los cinco al emisor. Es el único cuello que se renegocia en vez de rediseñarse |
| **Quién es el destinatario de un aviso**, o cuántos avisos cuelga el backlog del canal | El factor de `S-14` y, en proporción inversa, el tramo del primer cuello: `4,9 → 5,0` lo bajó de `1 360` a `1 333`. Reatribuir un aviso de una punta a la otra —lo que pasó con RF-60, que ahora informa al emisor, y con RF-121, que informa al receptor— no mueve el total; **agregar uno sí, y siempre hacia abajo** |
| La proporción de giros rehechos tras perderse el código (`S-19`, anclada en RF-110, RF-117 y RF-123) | Las `2,13` ventanillas por giro, el tercer término del caudal de cumplimiento y `0,045` de las `5,0` unidades del canal. No mueve el volumen de negocio: mueve tres columnas de la tabla de tramos sin mover la primera |
