...
lo nace por mucho más**: la cuota del operador de telefonía se agota en `831` giros diarios y `E`
estima la llegada en `2 800`, **`3,4×` por encima**. El canal carga hoy **veinticuatro** requerimientos
—trece al receptor y once al emisor, más dos reglas de redespacho y una prohibición— y el factor de
`S-14` subió de `6,5` a `8,02` unidades por giro, con lo que el cuello bajó de `1 026` a `831`. **De
ese salto, el 93 % no son ítems nuevos sino una omisión corregida**: `RF-91`, `RF-156`, `RF-172`,
`RF-178`, `RF-190` y `RF-199` despachan por el canal desde hace rondas y `S-14` no los contaba. (2) El plano único
de escritura que [`D`](../D-disenar-servicio/README.md)(a) eligió deja de caber en una máquina en
**`19 100` giros/día** —era `21 000`, y lo bajaron las `32,82` escrituras por giro de hoy contra las
`29,96` de la ronda anterior—, todavía entre el segundo tramo y el tercero, y mucho antes de que
...
| **Consume de D** | Las **seis** decisiones, ya cerradas. Tres tienen tramo de vencimiento y aquí se nombra cuál: **(a)** el libro de partida doble solo-adición en un plano único de escritura, que deja de caber en una máquina en `19 100` giros/día; **(c)** el arrendamiento exclusivo solo en línea, que **no** vence —elimina la divergencia por construcción— pero convierte cada caída de enlace en mostrador cerrado, y esa cuenta crece con los puntos; **(f)** el servicio de desafío, que `D` puso con base y credenciales propias y al que `E` **sí le da nodo**: es el **quinto plano** de su tabla de trece servidores, con `2`, y RNF-15 y RF-114 lo ensancharon al sacar de la pantalla del mostrador tanto el código como la respuesta. Lo que sigue abierto no es si tiene nodo, sino si `2` alcanzan cuando el `20 %` de los pagos pasa por ahí |
...
**diario** de `P`, no el compuesto de `11`), `30,4 / 2` giros por emisor y mes (`S-04`), **`8,02`
avisos por giro** (`S-14`, contra los **veinticuatro** ítems que el canal carga hoy: RF-25, RF-37,
RF-41, RF-69, RF-75, RF-106, RF-109, RF-120, RF-121, RF-124, RF-156, RF-178, RF-190 y RF-199 al
receptor; RF-11, RF-14, RF-51, RF-60, RF-65, RF-67, RF-91, RF-127, RF-172, RF-231 y RF-232 al emisor,
más el redespacho de RF-202 y RF-227) y `0,96` pagos por giro (`S-13`).
...
|---|---|---|---|---|---|---|
| `< 1 k` | **2 130** — menos que las `9 000` declaraciones de caja del mismo día | **3 000** | **15 200** | **3 000** — piso de `S-12`, no derivado del volumen | **8 020** — ya por encima del `40 %` de la cuota, en el tramo más bajo del cuadro | **2** |
| `1 k – 10 k` | **21 300** — SendIt entra acá, en `2 800` (`5 955` ventanillas) | **30 000** | **152 000** | **3 000** — la red sigue al `11 %` de su techo de caja | **80 200** | **2** |
| `100 k` | **213 000** | **300 000** | **1 520 000** | **3 200** — primer tramo en que la red deja de estar sobrada | **802 000** | **2**, al `18 %` de los dos corredores enteros |
| `500 k` | **1 065 000** | **1 500 000** | **7 600 000** | **16 000** | **4 010 000** | **≥ 12** — `89 %` de los dos corredores es inalcanzable |
| `1 M` | **2 130 000** | **3 000 000** | **15 200 000** | **32 000** | **8 020 000** | **≥ 24** — `1,8×` los dos corredores enteros |
...
entera** —de `6,5` a `8,02` unidades por giro— y ese `23 %` es lo que separa `1 026` de `831`
...
## Los cinco tramos del material, en su unidad y en su forma

El capítulo 3 dibuja este paso de una manera concreta —láminas 40 a 45— y este documento la reproduce
antes de meterse en lo suyo: **el mismo diagrama, cinco veces, cada vez con una caja más**, rotulado
en **usuarios** y no en giros. Después viene el análisis por `giros/día`, que es la unidad en la que
`E` estimó y en la que las roturas de este caso se dejan medir.

**Dónde cae SendIt en esta escala, que es lo primero que hay que decir.** `S-02` cuenta **42 500
emisores distintos al mes** para los `2 800` giros diarios del lanzamiento. SendIt **no arranca en el
primer tramo del material: arranca en el tercero**, el de `10K – 100K`. La arquitectura de la lámina
40 —un servidor y una base— nunca es la de este sistema, ni el día uno.
...
| `1K – 10K` usuarios | La DB es el cuello | Load Balancer + varios app servers + réplica | **La DB no es el cuello de SendIt en ningún tramo.** Lo que aparece acá es el piso de `RF-76`: `9 000` declaraciones de caja diarias que no dependen del volumen |
| `10K – 100K` usuarios | Lecturas muy altas | Cache + CDN para estáticos | **Acá arranca SendIt** (`42 500`). Y la caché del material **no aplica al dato caliente**: `RNF-05` y `RNF-07` exigen leer del libro. Lo cacheable son catálogos —puntos, corredores, listas— y eso es lo que `L` cachea |
| `100K – 1M` usuarios | Cache miss, escrituras lentas | Sharding + réplicas + regiones | **El tramo real de SendIt**, y llega por la puerta que el material anticipa: el plano único de escritura de `D`(a) deja de caber en una máquina en `19 100` giros/día. La partición es **por giro**, y `RNF-07` decide que sea esa y no otra |
| `> 1M` usuarios | Todo a la vez | Microservicios + event-driven + multi-región + federación | **No se alcanza con arquitectura.** `1 M` giros/día son `USD 109 500 M/año`, `1,8×` los dos corredores enteros. Lo que hay que cambiar antes que las cajas son los supuestos de `E` |
...
|---|---|---|
| Qué es | Más máquinas del mismo tipo | Una máquina más grande |
| Requisito | Que la aplicación sea *stateless* | Ninguno |
| **En SendIt** | **Los servicios de aplicación, sí**: `D` los dejó sin estado y el arrendamiento de `D`(c) vive en el centro, no en el proceso | **El libro de `D`(a), hasta `19 100` giros/día**: es un escritor único por diseño, porque `RNF-07` pide que el asiento y el estado del giro quepan en una transacción |
| Qué obliga a cambiar de estrategia | — | Cuando la vertical se acaba, la partición **por giro** es la única que `RNF-07` admite: dos giros nunca comparten transacción, un giro nunca se parte |

**Y una tercera que el material no tiene porque su ejemplo no la necesita:** SendIt escala también
por **red afiliada**, que no es cómputo. `S-12` pone `3 000` puntos; el efectivo que cada uno sostiene
(`RF-20`, `RF-95`) y las `9 000` declaraciones de caja de `RF-76` crecen con esa columna y no con la
de usuarios. Un tramo se puede alcanzar por volumen o por ensanchamiento de la red, y no se arreglan
igual: el primero se compra con máquinas, el segundo con tesorería.
...
| **Nada de máquina** | La carga pide `0,15 × 1 000 / 2 800 = ` **`0,05`** servidores y RNF-11 con la geografía piden `13`. El sistema arranca sobreprovisionado `13 ÷ 0,05 = ` **`×260`** y sigue estándolo todo el tramo. Las `23 358` escrituras que el backlog de 155 ítems agregó subieron la carga un `34 %` y no movieron una caja — el sobreprovisionamiento bajó de `×360` a `×260` y sigue siendo de dos órdenes de magnitud | Ninguno. Subir dentro de este tramo no cambia una sola caja del diagrama |
...
| **Nada de máquina, otra vez** | En el techo del tramo la escritura pide `10 000 × 32,82 ÷ 86 400 × 11 ÷ 80 = ` **`0,52`** servidores en pico contra los `80 req/s` de un plano — era `0,48` con las `32,82` escrituras por giro de la ronda anterior | Ninguno. Los **trece** servidores siguen siendo de RNF-11 y de la geografía, no de la carga |
...
|---|---|---|---|
| 1 | La cuota del operador de telefonía | `831` giros/día — **`2,7×` debajo del volumen de llegada** | Un contrato con más cuota, no una máquina — y **no una cola que descarte**: ningún título permite no avisar |
...
adelantó de `21 000` a `19 100` giros/día sin dejar de ser el tercero. **El orden no cambió con los
236 ítems; las distancias sí**, y todas hacia abajo.

> **Deuda de repropagación: saldada.** Este paso estuvo doce rondas calculado sobre el backlog de **155 ítems**. Se rehízo contra los **236** vigentes, y además **se alineó a la forma que el material pide**: las cinco láminas del capítulo 3 —el mismo diagrama creciendo, rotulado en usuarios— están arriba, con la tabla de tres columnas del propio material y la comparación *scale out* / *scale up* de la lámina 46. El análisis por `giros/día` que sigue es la capa propia de este caso, no un reemplazo de aquella. **Ningún tramo cambió de orden; los cuellos se adelantaron**: el canal de `1 026` a `831` giros/día y el plano de escritura de `21 000` a `19 100`.
...
que ocurre **antes** del pago. De ahí salen las **`32,82`** escrituras por giro que `E` desglosó —eran
`29,96` antes de que el backlog de 236 ítems agregara `2,86` entre **seis** partidas, de las que una
sola, la constancia de aviso con su prueba de entrega (RF-98, RF-131, RF-132), aporta `2,74`—, y de
ahí sale el tramo: `32,82 × giros ÷ 86 400 × 11 = 80 req/s` en **`19 100` giros/día**.
...
| Red afiliada, operador de telefonía, listas y forma de identidad válida | Constante por corredor: un contrato de afiliación (RF-24), un operador (RF-25), una fuente de listas (RF-26, RF-119, RF-27) y un catálogo de documento (RF-01, RF-86, RF-122) | **No en cantidad; sí en precio unitario**: el operador nuevo se contrata contra `8,02` unidades por giro, no contra `5,0`, y ese factor lo fija el backlog, no el corredor |
...
| **La decisión `D`(a)** — el asiento con contrapartida para todo hecho del giro | El tramo de `19 100` giros/día en que el plano único de escritura deja de caber, que es función directa de las `32,82` escrituras por giro. Menos asientos por giro corre el tramo hacia arriba; más, hacia abajo — y las `8,34` que el backlog de 155 ítems agregó ya lo corrieron `8 100` giros/día hacia abajo |
...
| **La constancia de aviso y su prueba de entrega (RF-98, RF-131, RF-132)** | El tramo de `19 100` giros/día del cuello 3: son `18 760` de las `23 358` escrituras que el backlog agregó, y solas explican `6,70` de las `8,34` escrituras por giro nuevas. **Es la primera vez que el tramo de escritura lo mueve algo que no es dinero ni identidad.** Y se devuelve a `D` y a `L`: `E` (estimar) lo dejó anotado como candidato a **sexto plano**, porque la familia de escritura más grande del sistema vive hoy en el plano del giro, no se indexa por giro y tiene estado propio —*no avisado*— que RF-132 obliga a poder derivar |
| La cuota, el precio o la cobertura del operador de telefonía (`S-14`) | El cuello del canal de voz y SMS —`20 000 ÷ (6,5 × 3,0) = 831` giros/día— y con él la viabilidad de los **once** avisos al receptor y los **siete** al emisor. Es el único cuello que se renegocia en vez de rediseñarse — **y el único que este documento intentó una vez resolver descartando avisos, que es lo que ningún título permite** |
| **Quién es el destinatario de un aviso**, o cuántos avisos cuelga el backlog del canal | El factor de `S-14` y, en proporción inversa, el tramo del primer cuello: `5,0 → 6,5` lo bajó de `1 026` a `831`. Reatribuir un aviso de una punta a la otra —lo que pasó con RF-60, que informa al emisor, y con RF-121, que informa al receptor— no mueve el total; **agregar uno sí, y siempre hacia abajo — y dejar uno sin contar lo baja igual, en silencio, que es lo que pasó con RF-11** |
| La proporción de giros rehechos tras perderse el código (`S-19`, anclada en RF-46, RF-117, RF-123, RF-134 y RF-135) | Las `2,13` ventanillas por giro, el tercer término del caudal de cumplimiento (el `1` de los `71`) y `0,060` de las `8,02` unidades del canal —`0,045` de base repetida más `0,015` del código nuevo de RF-11—. No mueve el volumen de negocio: mueve tres columnas de la tabla de tramos sin mover la primera |
