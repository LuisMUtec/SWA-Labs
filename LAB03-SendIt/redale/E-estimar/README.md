...
| Traspasar el código al emisor y recibir la respuesta del propio receptor | escritura ilegible **dentro del módulo del desafío** | RF-11, RF-113, RF-114, RF-125, RF-147, RNF-15, RNF-17 | 4 | **3 696** = `2 800 (RF-113, un traspaso por giro) + 896 (RF-114, una captura por comprobación de S-16)`. RF-125 —confirmarle al operario que el código salió sin mostrárselo— viaja en la respuesta del mismo acto y **no agrega escritura durable** | Escritura |
...
| Caso de cumplimiento y su evidencia de liberación (RF-33, RF-87, RF-163, RF-127) | Motivo, derivación al segundo rol (RF-30), plazo del regulador más estricto (RF-31), el motivo de la liberación que RF-87 exige, lo que RF-156 le informa a quien dejó de poder cobrar y el aviso de RF-127 al emisor cuando el giro deja de estar retenido | 40 KB | `0,0254` = 2 % de S-08 `+ 0,5 %` de RF-93 `+ 2 %` de los `42` rehechos de RF-123 → **71 casos/día** | **2,8 MB** | 8,7 | `[ASSUMPTION: S-08]` |
...
| Traspaso del código y captura de la respuesta (RF-113, RF-114) | Constancia de que el código llegó al emisor **sin pasar por la pantalla del mostrador** y de que la respuesta la entró el propio receptor. Nunca el código ni la respuesta: RNF-15 y RNF-17 los mantienen ilegibles, y RNF-15 lo mantiene no reconsultable para el mostrador | 0,3 KB | `1,32` = `1` (RF-113) `+ 0,32` (RF-114, S-16) | **1,1 MB** | 8,7 | `[ASSUMPTION]` |
