...
**Estado: calculado sobre los 236 ítems del backlog** (216 RF + 20 RNF).

> **Deuda de repropagación: saldada.** Este paso estuvo doce rondas calculado sobre los **155 ítems**
> de la ronda 11, con la deuda declarada en la cabecera. Se rehízo entero contra el backlog vigente de
> **236**: los veintiún títulos que las rondas 17 a 19 agregaron (`RF-214`…`RF-235`) entraron a los
> tres cálculos, y `RF-150` salió sin dejar carga porque era una regla y no una operación.
>
> **Y el rebarrido encontró un defecto más viejo que los veintiún títulos: `S-14` enumeraba dieciocho
> avisos y el backlog despachaba veinticuatro.** Faltaban `RF-91`, `RF-156`, `RF-172`, `RF-178`,
> `RF-190` y `RF-199` — seis unidades del canal que el sistema paga y esta estimación no contaba. El
> factor sube de `6,5` a **`8,02` unidades por giro**, y con él **el primer cuello de botella del
> sistema empeora un 19 %**: la cuota del operador de telefonía se agota en `831` giros diarios, no en
> `1 026`. Contra los `2 800` de llegada, el sistema nace **3,4×** por encima de su primer cuello y no
> `2,7×`. La cifra de servidores también se movió, por primera vez en el proyecto: `0,139 → 0,153`.

Las tres cifras están abajo,
con su aritmética a la vista: **13 servidores**, **3,3 TB retenidos** (1,03 GB/día) y **1,7 Mbit/s en
pico** (209 KB/s). Ninguna sale del enunciado —el enunciado no da ni una cifra— y por eso cada
...
**El hallazgo que este paso entrega a `D`, antes que las cifras:** la carga de SendIt pide `0,15`
...
**Y un hallazgo que el backlog de 155 ítems produjo y el de 236 confirma:** la partida de escritura más
grande del sistema ya no es el libro contable sino la **constancia de aviso**. RF-98 conserva constancia
de cada aviso enviado, RF-131 registra si llegó cuando el canal lo reporta y RF-132 trata como **no
avisado** a quien no tenga constancia de que llegó. Entre los tres son `40 441` de las `91 887`
escrituras diarias — el `44 %`. El sistema escribe más sobre lo que dijo que sobre lo que debe.

**El hallazgo propio de esta repropagación es del otro lado del libro, y lo produce un ítem solo.**
`RF-221` —*«el sistema mostrará al operario en qué estado quedó la operación que acaba de cerrar»*—
es una lectura por cada atención de ventanilla: `5 955` al día, el `68 %` de las `8 771` lecturas que
los veintiún títulos nuevos agregaron y **el 35 % de toda la familia de consulta de estado**. Es el
precio de cerrarle a Kevin el hueco que arrastró desde la ronda 06: un título de una línea que no
cambia ninguna caja y sube la carga de lectura un `27 %`.
...
| **Avisos por llamada o SMS por día** | Unidades de voz o SMS entregadas a un tercero, **al receptor y al emisor**. Al receptor: RF-25, RF-37, RF-41, RF-69, RF-75, RF-106, RF-156, RF-109, RF-120, RF-121, RF-124, **RF-232**. Al emisor: RF-11, RF-14, RF-51, RF-60, RF-65, RF-67, RF-127, **RF-231, RF-232**. Redespacho a pedido: **RF-227**, con **RF-228** excluyendo el aviso que lleva el código. Se pagan por unidad, no por byte | **22 467** | `8,02 unidades por giro × 2 800 = 22 467` — desglose en S-14. A USD 0,05 la unidad: `22 467 × 0,05 = USD 1 123/día`, `× 365 = ` **USD 410 k/año**, `1 123 / 2 800 = ` **USD 0,401 por giro** | `[ASSUMPTION: S-14]` |
| Giros por día en pico | `giros por día × P` | **8 400** | `2 800 × 3,0` — el componente **diario** de `P`, no el compuesto | `[ASSUMPTION: S-07]` |
| RPS — requests por segundo | Derivada, no supuesta: sale del desglose por operación del cálculo 1 | **1,53 promedio · 16,9 en pico** | `132 564 requests/día ÷ 86 400 s = 1,534` · `1,534 × 11 = 16,88` | Derivado |
...
| S-14 | Avisos por giro —al receptor **y** al emisor—, y coste por unidad del canal | El canal de voz y SMS es de un tercero y se paga **por unidad entregada**, no por byte: es la única partida del paso cuyo costo no baja aunque el mensaje quepa en 140 caracteres. Un giro sin incidente ya gasta cuatro al receptor (RF-25, RF-41, RF-75, RF-199) y uno al emisor —el código de RF-11—; cada desenlace agrega el suyo. RF-132 hace del reintento una obligación y no una cortesía: **quien no tenga constancia de que el aviso llegó cuenta como no avisado**, así que la unidad se repite hasta que RF-131 registre la entrega | La cuota que un operador de telefonía concede por país, que RNF-12 obliga a contar por país de origen y de destino | **8,02 unidades por giro · USD 0,05 por unidad.** Al receptor: `4,0` de base (RF-25 hay un giro, RF-41 el monto, RF-75 si el punto tiene efectivo, **RF-199 en qué punto cobrar**) `+ 0,060` la base repetida sobre el giro rehecho (`1,5 % × 4`, S-19) `+ 0,20` pregunta del desafío (RF-69) `+ 0,0254` retenido (RF-37, sobre los 71 casos de S-08) `+ 0,01` retenido por cambio de lista (RF-37 sobre RF-97) `+ 0,02` el punto dejó de poder pagarle (RF-109) `+ 0,02` a qué otro punto ir (RF-124) `+ 0,04` devuelto al emisor (RF-106, S-13) `+ 0,005` prescripción próxima (RF-120) `+ 0,005` diferencia registrada (RF-121) `+ 0,035` **quedó no pagable (RF-190)** `+ 0,05` **confirmar que el código corresponde (RF-178)** `[ASSUMPTION: lo usa el 5 % de los receptores]` `+ 0,005` **reposición disponible (RF-156)** `+ 0,015` **vuelve a ser pagable (RF-232)** `= 4,490`. Al emisor: `1,015` el código de seguimiento (RF-11, uno por giro creado **más uno por giro rehecho**) `+ 0,96` cobrado (RF-65) `+ 0,04` devolución disponible (RF-14) `+ 0,0254` retenido (RF-67) `+ 0,018` giro liberado (RF-127) `+ 0,005` prescripción próxima (RF-51) `+ 0,005` diferencia (RF-60) `+ 0,019` **otro punto de atención (RF-91, uno por rederivación)** `+ 0,035` **quedó no pagable (RF-172)** `+ 0,035` **la acción ofrecible de ese giro (RF-231)** `+ 0,015` **vuelve a ser pagable (RF-232)** `= 2,173`. Suma `6,663`, `× 1,20` de reintentos de RF-202 `= 7,996`, `+ 0,028` de los redespachos a pedido de RF-227 `= ` **8,024 → 8,02**. **Los `0,035` de no pagable salen de `98` giros/día** = `71` retenidos (S-08) `+ 27` cuyo documento no coincide (RF-230, `[ASSUMPTION: 1 % de los pagos intentados]`); los `0,015` de RF-232, de las `43` retenciones que se liberan. **El delta contra el `6,5` de la ronda anterior son `1,52` unidades, y solo `0,10` son títulos nuevos:** `1,42` los seis avisos que la enumeración omitía —RF-199 vale `1,0` él solo, más su parte del rehecho— y `0,10` entre RF-231, RF-232 y RF-227. **Y RF-115, RF-116, RF-136 y RF-142 siguen sin entrar acá**: son plazos y cifras que se informan en la ventanilla, no unidades del canal | `[ASSUMPTION]` · `[ASSUMPTION: reintento de 1,20 — 12 % por entrega fallida más 8 % por entrega sin constancia, que RF-132 obliga a tratar como no avisado]` · `[ASSUMPTION: 70 % de las retenciones se liberan y disparan RF-127; el 30 % vence y sale por RF-32]` · `[ASSUMPTION: 2 % de los avisos sin reporte de entrega se reclaman por RF-227, con RF-228 excluyendo el del código]` |
...
| 3. Carga objetivo | `RPS derivado del desglose de abajo × P` | escritura: `91 887 / 86 400 = 1,063 req/s`, `× 11 = ` **11,70 req/s** · lectura: `40 677 / 86 400 = 0,471 req/s`, `× 11 = ` **5,18 req/s** | Derivado |
| 4. Servidores | `carga objetivo / capacidad de servidor` | escritura: `11,70 / 80 = 0,146` · lectura: `5,18 / 800 = 0,006` · suma: **0,153 → 0,15 servidores** | Derivado — **un solo servidor cubre seis veces y media la carga del pico**. Era `0,10` sobre 137 ítems y `0,139` sobre 155. **Es la primera vez que la cifra mueve su segunda decimal**, y no la mueven los ítems nuevos: la mueven los seis avisos que `S-14` no contaba |

Donde el material obtiene `100 000 / 160 = 625 servidores`, SendIt obtiene `11,70 / 80 = 0,146`. No es
...
su tasa de fallo no las fija este sistema. **Veinticuatro requerimientos despachan por él, y esta
repropagación tuvo que contarlos de nuevo porque la cuenta anterior decía dieciocho:** **trece del
lado del receptor** —RF-25, RF-37, RF-41, RF-69, RF-75, RF-106, RF-109, RF-124, RF-156, RF-178,
RF-190 y RF-199 en P0, más RF-120 y RF-121 en P1— y **once del lado del emisor** —RF-11, RF-14,
RF-65, RF-67, RF-91, RF-127, RF-172, RF-231 y RF-232 en P0, más RF-51 y RF-60 en P1—. A eso se suman
**dos reglas de redespacho** —RF-202, automático cuando el canal reporta no entregado, y RF-227, a
pedido del destinatario identificado— y **una prohibición que acota a las dos**: RF-228 excluye del
redespacho el aviso que lleva el código.

**Seis de esos avisos no son títulos nuevos: llevaban rondas escritos y esta estimación no los
contaba.** `RF-91` ofrece otro punto por llamada, `RF-199` dice en cuál cobrar, `RF-156` avisa que la
reposición está disponible, `RF-172` y `RF-190` avisan a cada punta que el giro quedó no pagable, y
`RF-178` le confirma al receptor que su código corresponde a un giro suyo. Son unidades que el
operador factura y que el cuello de botella soporta. **El error no estaba en la aritmética sino en la
enumeración**, que es justo la clase de defecto que un `grep` no delata: la fórmula reproducía su
propio total sin fallar, sobre una lista incompleta.
...
| Avisar por llamada o mensaje de texto (receptor y emisor) | **salida por canal de un tercero, por unidad** | Receptor: RF-25, RF-37, RF-41, RF-69, RF-75, RF-106, RF-156, RF-109, RF-120, RF-121, RF-124 · Emisor: RF-11, RF-14, RF-51, RF-60, RF-65, RF-67, RF-127 | **No se mide en requests** | **22 467 unidades** entregadas al tercero (`8,02 × 2 800`, S-14), más **40 441** escrituras propias: `22 467` constancias de envío (RF-98) y `17 974` registros de resultado de entrega (RF-131, sobre el `[ASSUMPTION: 80 %]` de unidades cuyo canal reporta resultado). RF-132 hace de la ausencia de constancia un estado —**no avisado**— y no una escritura más | Unidad de tercero + escritura. **Es la familia de escritura más grande del sistema: el 44 % del total** |
...
| Mostrar al operario el estado de la operación que acaba de cerrar | lectura | RF-221 | 1 | **5 955** — una por cada atención de ventanilla | Lectura — **nueva, y la mayor de las nuevas**: sube la carga de lectura un `19 %` con un título de una línea |
| Producir el plazo más largo que puede tomar una devolución | lectura agregada sobre las causas | RF-234, RF-73 | 2 | **2 800** — una por giro, porque RF-73 lo informa antes de que el emisor entregue el efectivo | Lectura — **nueva**. La cifra sin productor que RF-73 usaba desde la ronda 03 |
| Resolver con un segundo rol la duda sobre la fotografía del documento | escritura + segunda identidad | RF-225, RF-229 | 6 | **102** = `89` derivaciones `[ASSUMPTION: 3 % de las identificaciones abren duda]` `+ 13` rechazos de las que no se resuelven en `[ASSUMPTION: 10 minutos]` | Escritura — **nueva**. RF-229 la resuelve **antes** de que el operario reciba el efectivo, así que no hay nada que revertir |
| Derivar al rol de cumplimiento la identificación del receptor que agotó los intentos | escritura de apertura + escritura de cierre | RF-218, RF-219, RF-223 | 10 | **86** = `43` derivaciones `[ASSUMPTION: 8 % de los 538 pagos por desafío agota los tres intentos de RF-82]` `+ 43` cierres dentro de `[ASSUMPTION: 24 horas]`, de los que `34` habilitan el pago (RF-218) y `9` devuelven al emisor (RF-223) | Escritura — **nueva**. Es el único componente que los ítems de las rondas 17-19 obligan a crear |
| Dejar no pagable el giro cuyo documento no coincide con el receptor designado | escritura de estado | RF-230, RF-220 | 4 | **27** = `2 690 × 1 %` `[ASSUMPTION]` | Escritura — **nueva** |
| Identificar a quien cancela, suprime o cobra una reposición en el mostrador | escritura de identificación | RF-226, RF-215, RF-233, RF-222 | 6 | **27** = `22` cancelaciones presenciales `[ASSUMPTION: 40 % de las devoluciones nacen de una cancelación, la mitad pedidas en el mostrador]` `+ 4` reposiciones con documento vencido contra el desafío (RF-222) `+ 1,3` supresiones (S-18) | Escritura — **nueva**. Son las compuertas que la ronda 15 encontró abiertas del lado del dinero que sale |
| Comprobar el techo de una reposición contra la evidencia de entrega | lectura de la evidencia firmada | RF-214, RF-168 | 3 | **13** — una por diferencia declarada (S-17) | Lectura — **nueva**. Es lo que convierte a RF-168 en acto con consumidor |
| Producir el efectivo disponible de cada corredor a partir de lo declarado por sus puntos | lectura agregada | RF-235, RF-76 | 3 | **3** = `2 corredores × 1,5 turnos` | Lectura — **nueva**. Otra cifra que antes no tenía productor |
| Atender el redespacho de un aviso a pedido de su destinatario identificado | escritura de identificación + unidad de tercero | RF-227, RF-228 | 4 | **78** identificaciones del reclamante `[ASSUMPTION: 2 % de los avisos sin reporte de entrega]`. Sus unidades del canal están en la fila del aviso | Escritura — **nueva**. RF-228 excluye de este camino el aviso que lleva el código |
...
**Suma del desglose:** `132 564 requests/día`, repartidos en **91 887 escrituras** y **40 677
...
|---|---|
| **Familia de aviso recontada**: los seis avisos que `S-14` omitía (RF-91, RF-156, RF-172, RF-178, RF-190, RF-199) más los tres nuevos (RF-231, RF-232, RF-227), con su constancia de RF-98 y su resultado de RF-131 | `+7 681` |
| Duda sobre la fotografía del documento y su rechazo por vencimiento (RF-225, RF-229) | `+102` |
| Identificación derivada al rol de cumplimiento: apertura y cierre (RF-218, RF-219, RF-223) | `+86` |
| Identificación de quien pide el redespacho de un aviso (RF-227) | `+78` |
| Giro no pagable por documento que no coincide (RF-230) | `+27` |
| Identificación de quien cancela, suprime o cobra una reposición en el mostrador (RF-226, RF-215, RF-233, RF-222) | `+27` |
| **Total** | **`+8 001`** → `91 887` |

**Del lado de la lectura el delta es `+8 771`, y ahí sí mandan los ítems nuevos** — pero mandan tres
de veintiuno: `+5 955` de RF-221, que le muestra al operario en qué estado quedó la operación que
acaba de cerrar; `+2 800` de RF-234, que produce el plazo más largo de una devolución que RF-73
llevaba seis rondas informando sin que nada lo calculara; `+13` de RF-214 y `+3` de RF-235.
`31 906 → 40 677`. **Las dos cifras que más se movieron en este paso son un aviso que nadie contaba y
una lectura que nadie pedía**, y ninguna de las dos cambia una caja de `D` ni una entidad de `A`.
...
`503 414 / 10 = 50 341` escrituras equivalentes —**menos** que el recuento plano de 91 887, porque la
familia más numerosa, la constancia de aviso, es la más liviana, y esta repropagación la hizo crecer
un `23 %` más— y las lecturas dan `45 302` equivalentes, `1,11×` el recuento plano de 40 677: **la
razón bajó de `1,38×` porque RF-221 agregó seis mil lecturas del peso más barato que hay.**
Ninguno de los dos mueve la cifra ni medio orden de magnitud:
`50 341 / 86 400 × 11 / 80 = 0,080` servidores de escritura. **El sistema sigue pidiendo menos de un
décimo de máquina por el camino ponderado, y quince centésimas por el plano.**

### Lo que sí dimensiona: RNF-11, RNF-10 y la geografía

`0,15` no es un número de servidores: es la prueba de que la carga no es la restricción activa. El
...
la constancia de aviso en la familia de escritura más grande del sistema —`40 441` de `91 887`, el
...
46 % de los dos corredores. **El backlog de 236 ítems acercó ese horizonte** —eran
...
| Aviso por llamada o SMS y **la prueba de que llegó** (receptor: RF-25, RF-37, RF-41, RF-69, RF-75, RF-106, RF-156, RF-109, RF-120, RF-121, RF-124 · emisor: RF-11, RF-14, RF-51, RF-60, RF-65, RF-67, RF-127) | Dos registros, no uno: la constancia de envío que RF-98 obliga a conservar de cada unidad entregada al tercero —destinatario, canal, momento— y el **resultado de entrega** que RF-131 registra cuando el canal lo reporta, sin el cual RF-132 obliga a tratar al destinatario como **no avisado** | 0,3 KB la constancia · 0,1 KB el resultado | `8,02` constancias (S-14) `+ 6,42` resultados (`80 %` de las unidades) | **8,5 MB** = `(8,02 × 0,3 + 6,42 × 0,1) × 2 800` | 8,7 | `[ASSUMPTION: S-14]` — **la partida que más creció, y no mueve un centavo** |
...
- **La asimetría se invierte respecto del ejemplo del material.** En YouTube la salida aplasta a la
  ...
  salida; SendIt `1 228,3 / 413,2 = 3,0×` a favor de la **entrada** — y la asimetría **se achicó**,
  ...
  evidencia, y es el `71 / 413,2 = ` 17 % de toda la salida generada por menos del 0,1 % de las
  operaciones.
...
|---|---|---|
| **La cuota de mercado del lanzamiento (0,5 %) o el ticket promedio (S-05)** | **Los tres cálculos, en proporción directa** | Nada, mientras no cruce un orden de magnitud: `0,15` servidores de carga aguantan `×93` antes de alcanzar el piso de 13. Es el supuesto del que más depende la cifra y el que menos cambia la arquitectura |
...
| **La constancia de aviso y su prueba de entrega (RF-98, RF-131, RF-132)** | El desglose de escrituras, que hoy le debe `40 441` de `91 887` — el `44 %` — y el espacio por giro, que le debe `0,58` de los `0,70 KB` que esta repropagación agregó | `D` y `L` — es el candidato a **sexto plano**: la familia de escritura más grande del sistema vive hoy en el plano del giro y tiene estado propio (*no avisado*), que RF-132 obliga a poder derivar |
