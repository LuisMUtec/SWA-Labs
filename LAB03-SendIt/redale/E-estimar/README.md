...
**Estado: calculado sobre los 155 ítems del backlog** (138 RF + 17 RNF). Las tres cifras están abajo,
...
declarar **por llamada o mensaje de texto**»*, y RF-126 lo confirma al exigir identificar como el
...
RF-37, RF-41, RF-69, RF-75, RF-106, RF-108, RF-109 y RF-124 en P0, más RF-120 y RF-121 en P1— y
...
ventanilla sino una declaración **por llamada o mensaje de texto**, y RF-126 obliga a identificar a
...
| Comprobar la respuesta del desafío sin mostrarla | lectura ilegible, hasta `[ASSUMPTION: 3]` intentos por pago (RF-82) | RF-12, RF-69, RF-82 | 2 | **896** = `2 800 × 0,20 × 1,6` (S-16) | Lectura |
...
| Admitir una declaración de diferencia hecha fuera del mostrador | escritura + identificación remota + atribución al punto | RF-90, RF-126, RF-59 | 8 | **26** = `13` declaraciones (S-17) `+ 13` identificaciones del declarante (RF-126) | Escritura. **No es ventanilla**: RF-90 llega por llamada o SMS |
...
tamizaje (RF-119, RF-123) `+ 3 696` del traspaso del código y la respuesta (RF-113, RF-114) `+ 2 854`
...
| Identificación de quien declara una diferencia fuera del mostrador (RF-126) | `+13` |
...
| Caso de cumplimiento y su evidencia de liberación (RF-33, RF-87, RF-108, RF-127) | Motivo, derivación al segundo rol (RF-30), plazo del regulador más estricto (RF-31), el motivo de la liberación que RF-87 exige, lo que RF-108 le informa a quien dejó de poder cobrar y el aviso de RF-127 al emisor cuando el giro deja de estar retenido | 40 KB | `0,0254` = 2 % de S-08 `+ 0,5 %` de RF-93 `+ 2 %` de los `42` rehechos de RF-123 → **71 casos/día** | **2,8 MB** | 8,7 | `[ASSUMPTION: S-08]` |
...
| Declaración de diferencia del receptor, su atribución y su liquidación (RF-90, RF-126, RF-59, RF-104, RF-143, RF-145) | Lo que el receptor declara **por llamada o SMS**, la identificación que RF-126 exige antes de admitirla, la atribución al punto, el informe previo con evidencia que RF-104 obliga a darle al agente, el descuento de RF-143 y la objeción de RF-145 | 12 KB | `0,005` (S-17) | **0,17 MB** | 8,7 | `[ASSUMPTION: S-17]` |
