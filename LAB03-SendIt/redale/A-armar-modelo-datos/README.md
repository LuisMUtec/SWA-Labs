...
**Estado: entregado.** Treinta y tres entidades con campos concretos, motor elegido por entidad
...
| **Receptor** | Elena. No es un campo del giro: es una persona con identidad propia, que se verifica y que cobra contra código y documento (`RF-34`, `RF-85`). Modelarla como texto dentro del giro hace imposible reconocer que dos giros van a la misma persona, que es justo lo que el tamizaje necesita ver —`RF-119` la contrasta **antes** de aceptar el efectivo y `RF-27` otra vez al autorizar el pago— y lo que `RF-58` obliga a contar |
...
| **Declaración de diferencia del receptor** | La voz de Elena en el mostrador: `RF-90` la deja declarar que el dinero recibido no coincide con el informado. Es un hecho asentado por quien no es usuaria del sistema, contra un monto que `RNF-16` declara intocable y una diferencia que `RF-59` atribuye al punto donde se entregó el dinero. Sin entidad, esa declaración solo existe si el operario la escribe — y es sobre él que se declara |
...
| **Recepción de efectivo en custodia** | El efectivo que Kevin ya contó y guardó cuando la creación se cortó antes de existir el giro. `RF-118` obliga a dejarlo *en un estado único que el operario puede leer y retomar*, y ese estado no puede ser una clave de intento con vencimiento: `BR-01` vuelve ese dinero deuda de SendIt desde que entra al cajón, y una deuda que expira sola es una deuda que alguien se quedó |
| **Acción ofrecible** | El catálogo que `RF-39` volvió obligatorio al pedir que **todo** giro rechazado, retenido o no pagable **lleve asociada al menos una acción** que mostrarle al operario. Un texto compuesto en la pantalla no se puede auditar por cobertura ni versionar, y sobre todo no se puede comprobar que exista para los tres estados |
...
| **Clave de idempotencia** | **Entidad propia**, en `BD intentos`, con vida acotada y motor distinto del resto. `RF-43` y `RF-44` obligan a tratar el reintento como la misma operación, y para eso la clave tiene que persistirse **antes** de mover un centavo y devolver el resultado original —no un error— al segundo intento. Es la única entidad del modelo que **caduca sola y no deja registro contable**, y por eso mismo **no puede cargar con `RF-42` ni con `RF-118`**: un estado que el operario debe poder leer y retomar no se puede confiar a una fila que se borra sola. Lo interrumpido vive en `Giro.estado` y en `Recepción de efectivo en custodia`; la clave solo dice que el reintento es el mismo intento | `RF-43`, `RF-44`, `RF-117` |
| **Aviso enviado** | **Entidad propia**, y ya no por conveniencia: `RF-98` obliga a conservar constancia de cada aviso **y a oponerla a quien alegue no haber sido avisado**. Son **diecisiete** tipos colgando de ítems distintos, y los que la ronda partió llegan de a dos sobre el mismo hecho —la diferencia se avisa al emisor por `RF-60` y al receptor por `RF-121`; la prescripción, por `RF-51` y `RF-120`—, de modo que la clave natural es `(giro, tipo, destinatario)` y no `(giro, tipo)`. Un campo `avisado` no distingue cuál se envió, no distingue a quién, y sobre todo **no se opone a nadie**: oponer exige saber qué se dijo y que un tercero lo acuse | `RF-98`, `RF-121`, `RF-120`, `RF-106` |
...
| **Giro** · `BD giros` | `giro_id`, `emisor_ref`(FK), `receptor_ref`(FK), `jurisdiccion_origen_id`(FK), `jurisdiccion_destino_id`(FK), `version_regla_jurisdiccion` (congelada al crear), `monto_origen`, `moneda_origen`, `monto_destino`, `moneda_destino`, `comision`, `cotizacion_id`(FK), `punto_eleccion_id`(FK), `punto_pago_id`(FK), `estado` (proyección), `version_estado`, `disponible_desde`, `prescribe_en`, `conservar_hasta`, `plazo_aviso_retencion` (el que `RF-115` le informó al emisor **antes** del efectivo, congelado al crear), `accion_ofrecible_id`(FK, **obligatorio cuando `estado` es rechazado, retenido o no pagable**, `RF-39`), `rehecho_de_giro_id`(FK, nulo salvo `RF-117`), `comision_ya_cobrada` (bool, `RF-117`), `custodia_id`(FK, nulo salvo cuando nació de una custodia de `RF-118`), `clave_idempotencia`(FK), `creado_en`, `creada_por_identidad_id`(FK) | `giro_id` | 1→1 `Código de seguimiento`, `Desafío`, `Prescripción` · 1→0..1 `Pago`, `Devolución`, `Recepción de efectivo en custodia`, `Giro` (el rehecho, por `rehecho_de_giro_id`) · 1→N `Movimiento contable`, `Traza de auditoría`, `Aviso enviado`, `Resultado de tamizaje`, `Oferta de punto alternativo` |
| **Código de seguimiento** · `BD giros` | `giro_id`(PK,FK), `codigo_hmac` (verificador, llave en el módulo de claves), `llave_id`, `emitido_en`, `entregado_por` ∈ {`canal_voz_sms`,`impresion_sellada_dispositivo`} (`RF-113`), `acuse_entrega`, `consumido_en` | `giro_id`; `UNIQUE(codigo_hmac)` | 1→1 `Giro`. **Ninguna columna guarda el código ni parte de él en claro** (`RNF-15`), y no hay ruta de lectura: `RF-94` prohíbe reemitirlo y `RF-112` volver a consultarlo, así que la tabla se compara y no se lee. `entregado_por` prueba **por dónde salió**, nunca **qué salió** |
...
| **Pago** · `BD giros` | `pago_id`, `giro_id`(FK, **ÚNICO**), `punto_id`(FK), `identidad_operario_id`(FK), `monto_entregado`, `moneda`, `entregado_en`, `verificacion_receptor_id`(FK), `desafio_usado` (bool, nunca el valor), `asiento_id`(FK), `clave_idempotencia`(FK) | `pago_id`; `UNIQUE(giro_id)` | 1→1 `Giro` · 1→0..N `Declaración de diferencia del receptor`. **El `UNIQUE` es `RNF-05`**: no hay forma de escribir dos entregas del mismo giro, en ningún punto y en ningún momento |
...
| **Recepción de efectivo en custodia** · `BD giros` + asiento en `BD contable` | `custodia_id`, `punto_id`(FK), `identidad_operario_id`(FK), `emisor_ref`(FK, nulo si la identificación no llegó a cerrar), `monto`, `moneda`, `recibido_en`, `giro_id`(FK, nulo hasta que el giro se completa), `estado` ∈ {`abierta`,`aplicada_al_giro`,`devuelta`}, `asiento_id`(FK), `cerrada_en`, `clave_idempotencia`(FK) | `custodia_id` | 1→0..1 `Giro` · N→1 `Punto de atención` · 1→N `Movimiento contable`. **Es la entidad de `RF-118`, y existe porque el efectivo de Rosa ya entró al cajón**: `BR-01` lo vuelve pasivo de SendIt con giro o sin él. `estado` se cierra completando el giro o devolviendo el efectivo — **no hay transición por vencimiento** |
...
| **Retención** · `BD cumplimiento` | `retencion_id`, `giro_id`(FK), `caso_id`(FK, nulo cuando la origina `RF-93`), `origen` ∈ {`sancion`,`acumulado_receptor`}, `jurisdiccion_id`(FK, **la del regulador más estricto de los dos**), `version_regla` (la que el giro obedeció), `vence_en`, `avisar_al_emisor_hasta` (`abierto_en + giro.plazo_aviso_retencion`: la fecha absoluta que hace exigible la promesa de `RF-115` y vencible el aviso de `RF-67`), `avisado_al_emisor_en`, `estado`, `liberada_en` | `retencion_id` | 1→1 `Caso de cumplimiento` · N→1 `Giro` · N→1 `Jurisdicción` por `(jurisdiccion_id, version_regla)`. **La existencia y la fecha se sirven al emisor (`RF-66`); el motivo no está en esta tabla y por eso no se puede filtrar por descuido** |
| **Punto de atención** · `BD puntos de atención y efectivo` | `punto_id`, `agente_id`(FK), `nombre`, `pais`, `distrito` (`RF-91` ofrece **del mismo distrito**), `jurisdiccion_id`(FK), `direccion`, `habilitado_cobro`, `habilitado_devolucion`, `dispositivo_cliente_id` (la superficie que el operario no lee, `RF-113`, `RF-114`), `capacidad_pago_declarada`, `ultima_sincronizacion_en`, `estado_conexion` | `punto_id` | N→1 `Agente` · N→1 `Jurisdicción` · 1→N `Caja del punto`, `Pago`, `Traza de auditoría`, `Oferta de punto alternativo`. Es el destino de **todas** las claves foráneas `punto_*_id` del modelo: `punto_eleccion_id` y `punto_pago_id` del giro, `punto_retiro_id` de la devolución, `punto_captura_id` del receptor, `punto_atribuido_id` de la declaración de diferencia |
...
| **Aviso enviado** · `BD auditoría` | `aviso_id`, `secuencia`, `giro_id`(FK), `destinatario_ref`(FK), `rol` ∈ {`emisor`,`receptor`}, `tipo` (uno de los **diecisiete**, y el tipo decide el texto tanto como el destinatario: al emisor, `RF-14`, `RF-51`, `RF-60`, `RF-65`, `RF-67`, `RF-116`; al receptor, `RF-25`, `RF-37`, `RF-41`, `RF-75`, `RF-106`, `RF-107`, `RF-108`, `RF-109`, `RF-120`, `RF-121`, `RF-124`), `canal` ∈ {`llamada`,`sms`}, `telefono_token`, `enviado_en`, `texto_hash` (**qué se dijo**, no solo que se llamó), `acuse`, `acuse_operador_id` (el identificador que devuelve el operador del canal), `resultado` ∈ {`entregado`,`no_entregado`,`sin_acuse`}, `intentos`, `resumen_contenido`, `hash_anterior` | `aviso_id`; `UNIQUE(giro_id, tipo, destinatario_ref)` | N→1 `Giro`. `telefono_token` y no el número: la constancia de `RF-98` tiene que sobrevivir a la supresión del contacto, y sobrevive porque no lo contiene. **`RF-98` ya no pide solo constancia: pide una constancia que se *oponga* a quien alegue no haber sido avisado**, y eso son tres columnas más y no una frase — `texto_hash` prueba qué se dijo, `acuse_operador_id` lo prueba **con un tercero** y `resultado` distingue *avisado* de *intentado*, que es exactamente la distinción sobre la que discutiría Elena. Sin `resultado`, oponer la fila sería afirmar más de lo que la fila sabe |
...
| Código de seguimiento del giro (`RNF-15`, `RF-112`, `RF-113`) | **No en claro, en ninguna base** | `codigo_hmac` con la llave en el módulo. **Ninguna columna guarda el código ni una parte de él en claro**, ni siquiera para localizarlo: se compara, no se busca| Nadie. **Ningún servicio tiene ruta de lectura**: `Payout Service` compara y recibe un booleano | El mostrador teclea lo que el receptor trae y el centro responde coincide / no coincide. **La salida hacia el emisor no pasa por ahí**: `RF-113` la manda por el canal o la imprime sellada desde el dispositivo orientado al cliente, y `entregado_por` guarda por dónde salió, nunca qué salió | `RF-11` lo entrega solo al emisor, `RF-94` prohíbe reemitirlo aun al emisor que lo perdió, `RF-112` prohíbe que quien atiende vuelva a consultarlo y `RF-34` obliga a exigirlo para pagar. Las tres juntas describen **comparación sin lectura**, que no es una regla de pantalla: es una columna que guarda un verificador y ninguna que guarde el valor. El prefijo existe porque sin él la comparación barre la tabla, y cuatro caracteres no reconstruyen nada |
...
| Efectivo recibido sin giro creado (`RF-118`) | **Se agrega el hecho; se actualiza su estado, y no vence** | `RF-118`, `RF-42`, `RF-79`, `RNF-07`, `BR-01` | La recepción es un hecho con asiento (`INSERT` en `BD contable`) y la custodia es su estado abierto: recorre `abierta` → `aplicada_al_giro` o `devuelta` y **no tiene transición por vencimiento**, que es exactamente lo que la distingue de la clave de idempotencia con la que antes se despachaba. El efectivo de Rosa no puede dejar de existir porque pasó un TTL |
...
| Giro rehecho y su cotización (`RF-117`, `RF-123`) | **Se agrega un giro nuevo; no se reabre el cancelado** | `RF-117`, `RF-123`, `RF-110`, `RF-06` | Rehacer **no revive** el giro cancelado: crea otro con `rehecho_de_giro_id` apuntando al primero, copia la tasa congelada de su cotización —`giro_rehecho_id` en `Cotización`— y marca `comision_ya_cobrada` para que el asiento de comisión no se repita. Y `RF-123` obliga a que las filas de control se escriban **enteras otra vez**: tamizaje de las dos puntas, acumulados, segunda autorización si aplica. Reusar las del giro viejo sería tratar un giro nuevo como una continuación |
...
| Constancia de aviso enviado (`RF-98`) | **Se agrega, y con qué se dijo adentro** | `RF-98`, `RF-121`, `RF-60` | Prueba que se avisó **y se opone a quien alegue lo contrario**, que es lo que la ronda le agregó al ítem: para eso la fila lleva `texto_hash`, `acuse_operador_id` y `resultado`, y no solo `enviado_en`. Un campo `avisado` en el giro se sobrescribiría con el aviso siguiente y no probaría ninguno de los diecisiete; una fila sin acuse prueba que SendIt marcó el envío, que no es lo mismo que avisar |
...
| Declaración de diferencia del receptor (`RF-90`) | El del giro | No — es un acto, y los actos no se borran | `RF-90`, `RF-59`, `RNF-03` | La única versión de los hechos que no escribió el punto al que `RF-59` le atribuye la diferencia |
...
| Constancia de aviso enviado (`RF-98`) | **Hasta que el giro cierra y se saldan sus plazos.** Partición propia, la más corta de `BD auditoría` | No mientras corra; el **contacto** sí se borra, la constancia no | `RF-98`, `RF-121`, `RF-60` | La prueba **oponible** de haber avisado —`texto_hash`, `acuse_operador_id`, `resultado`—, que es lo que sostiene a SendIt cuando Elena dice que nadie la llamó. Es el ejemplo de que la retención se decide por dato: un SMS no merece los cinco años del asiento, y ponerlo ahí sería pagar conservación regulatoria por un mensaje |
...
| Recepción de efectivo en custodia (`RF-118`) | **El del giro que la cierra**; si se devuelve el efectivo, el de la devolución. **Nunca un TTL** | No — es dinero recibido, no un intento | `RF-118`, `RNF-07`, `RNF-03`, `BR-01` | La contrapartida del efectivo que Rosa entregó y que todavía no es giro. Es la fila que impide el peor error de este modelo: **que el dinero de Rosa expire con un plazo técnico** mientras el asiento que lo registra sigue en pie sin nada que lo explique |
...
frecuencia que el propio modelo impone: los dos plazos nuevos se miden en horas, así que el barrido
que los sirve corre **minutado y no diario**. Los ocho plazos y qué hace cada uno:
