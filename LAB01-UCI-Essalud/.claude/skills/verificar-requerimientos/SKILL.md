...
Ocho controles mecánicos sobre los entregables, más la preparación de la rama. Todos viven en
...
scripts/verificar.sh                    # los ocho controles, con detalle de cada falla
...
`verificar.sh` sale con código 0 cuando los ocho pasan y 1 cuando alguno falla. **Ningún puntaje se
...
E4 delega en `guardia-diff.sh`.

## Qué no verifica

Un control en verde no dice que el requerimiento sea bueno: dice que no incurre en el defecto que ese
control sabe detectar. Ninguno de los ocho juzga si el comportamiento enunciado le sirve a una
...
El resultado literal de los controles se transcribe en `Spec/Eval-Report.MD`, en la sección
«Controles de convenciones» de cada iteración.
